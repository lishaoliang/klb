...
#include "klbnet/klbws/klb_wsserve_conn.h"
...
{
    klb_wsserve_conn_t* p_wsserve = (klb_wsserve_conn_t*)p_conn->extra;
...
static int klb_wsserve_conn_send_normal(klb_netconn_t* p_conn, int packtype, uint32_t sequence, uint32_t uid, const uint8_t* p_head, int head_len, const uint8_t* p_body, int body_len)
{
    return 0;
}
...
static int klb_wsserve_conn_init(klb_netconn_t* p_conn)
{
    ...
    p_conn->vtable.ioctrl = klb_wsserve_conn_ioctrl; // 对连接进行控制操作: get/set,etc.
    p_conn->vtable.send_normal = klb_wsserve_conn_send_normal; // 调用者 发送常规数据包
    p_conn->vtable.send_media = klb_wsserve_conn_send_media; // 调用者 发送媒体数据包
    p_conn->vtable.on_send = klb_wsserve_conn_on_send; // 当网络上可以发送数据时
    p_conn->vtable.on_recv = klb_wsserve_conn_on_recv; // 当网络上可以接收数据时
    p_conn->vtable.on_msg = klb_wsserve_conn_on_msg; // 当网络上有消息传来时
...
klb_netconn_t* klb_wsserve_connect()
{
    return NULL;
}
...
klb_netconn_t* klb_wsserve_conn_create()
{
