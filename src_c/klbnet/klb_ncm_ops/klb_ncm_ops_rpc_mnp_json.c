...
typedef struct klb_ncm_ops_rpc_json_mnp_t_
{
    // ncm 参数
    struct
    {
...
#define KNORJM_PARSE_HEAD 0             ///< 解析mnp头
#define KNORJM_PARSE_BODY 1             ///< 解析mnp数据体
...
}klb_ncm_ops_rpc_json_mnp_t;


//////////////////////////////////////////////////////////////////////////

static int parse_klb_ncm_ops_rpc_json_mnp(klb_ncm_ops_rpc_json_mnp_t* p_ops, klb_socket_t* p_socket)
{
    ...
    if (KNORJM_PARSE_HEAD ==  p_ops->status_parse)
    {
        // 解析头
        ...
        p_ops->status_parse = KNORJM_PARSE_BODY;
    ...
    else
    {
        ...
        else if(KLB_MNP_RPC_JSON == p_ops->mnp.packtype)
        {
            // 加入缓存
            ...
            if (p_ops->remain_len <= 0 &&(KLB_MNP_FULL == p_ops->mnp.opt || KLB_MNP_END == p_ops->mnp.opt))
            {
                ...
                p_ops->lparam.cb_recv(p_ops->p_ncm, p_ops->protocol, p_ops->id, 0, KLB_NCM_PACK_RPC_JSON, p_bin);
        ...
        if (p_ops->remain_len <= 0)
        {
            ...
            p_ops->status_parse = KNORJM_PARSE_HEAD;
        }
...
/// @return void* 连接的指针
static void* create_klb_ncm_ops_rpc_json_mnp(klb_ncm_ops_lparam_t* p_lparam, klb_ncm_ops_wparam_t* p_wparam)
{
    klb_ncm_ops_rpc_json_mnp_t* p_ops = KLB_MALLOCZ(klb_ncm_ops_rpc_json_mnp_t, 1, 0);
...
/// @param [in] *ptr            连接的指针
/// @return 无
static void destroy_klb_ncm_ops_rpc_json_mnp(void* ptr)
{
    klb_ncm_ops_rpc_json_mnp_t* p_opt = (klb_ncm_ops_rpc_json_mnp_t*)ptr;
...
static int init_klb_ncm_ops_rpc_json_mnp(void* ptr, klb_socket_t* p_socket, const uint8_t* p_data, int data_len)
{
    klb_ncm_ops_rpc_json_mnp_t* p_ops = (klb_ncm_ops_rpc_json_mnp_t*)ptr;
    ...
    while (true)
    {
        if (0 != parse_klb_ncm_ops_rpc_json_mnp(p_ops, p_socket))
        {
            break;
        }
    }
...
/// @param [in] *ptr            ops对象
/// @return int
static int send_rpc_klb_ncm_ops_rpc_json_mnp(void* ptr, klb_socket_t* p_socket, int packtype, uint32_t sequence, uint32_t uid, const uint8_t* p_head, int head_len, const uint8_t* p_body, int body_len)
{
    klb_ncm_ops_rpc_json_mnp_t* p_ops = (klb_ncm_ops_rpc_json_mnp_t*)ptr;

    if (KLB_MNP_RPC_JSON != packtype)
    {
        return 0;
    }

    klb_buf_t* p_buf = klb_mnp_pack_data(KLB_MNP_RPC_JSON, sequence, uid, p_head, head_len, p_body, body_len);
    if (NULL != p_buf)
    {
...
/// @param [in] *ptr            连接的指针
/// @return int
static int on_send_klb_ncm_ops_rpc_json_mnp(void* ptr, klb_socket_t* p_socket, int64_t now)
{
    klb_ncm_ops_rpc_json_mnp_t* p_ops = (klb_ncm_ops_rpc_json_mnp_t*)ptr;
...
/// @param [in] *ptr            连接的指针
/// @return int
static int on_recv_klb_ncm_ops_rpc_json_mnp(void* ptr, klb_socket_t* p_socket, int64_t now)
{
    klb_ncm_ops_rpc_json_mnp_t* p_ops = (klb_ncm_ops_rpc_json_mnp_t*)ptr;
    ...
    while (true)
    {
        ...
        if (0 < r)
        {
            ...
            {
                if (KLB_SOCKET_OK != klb_socket_get_status(p_socket) ||
                    0 != parse_klb_ncm_ops_rpc_json_mnp(p_ops, p_socket))
                {
                    break;
                }
            }
...
static int on_proc_klb_ncm_ops_rpc_json_mnp(void* ptr, klb_socket_t* p_socket, int msg, int64_t now)
{
    klb_ncm_ops_rpc_json_mnp_t* p_ops = (klb_ncm_ops_rpc_json_mnp_t*)ptr;
...
{
    p_ops->cb_create = create_klb_ncm_ops_rpc_json_mnp;
    p_ops->cb_destroy = destroy_klb_ncm_ops_rpc_json_mnp;

    p_ops->cb_init = init_klb_ncm_ops_rpc_json_mnp;
    p_ops->cb_ctrl = NULL;

    p_ops->cb_send_media = NULL;
    p_ops->cb_send_normal = send_rpc_klb_ncm_ops_rpc_json_mnp;

    p_ops->on_send = on_send_klb_ncm_ops_rpc_json_mnp;
    p_ops->on_recv = on_recv_klb_ncm_ops_rpc_json_mnp;
    p_ops->on_proc = on_proc_klb_ncm_ops_rpc_json_mnp;
...
{
    klb_ncm_ops_t ops = { 0 };
    klb_ncm_init_ops_rpc_json_mnp(&ops);
