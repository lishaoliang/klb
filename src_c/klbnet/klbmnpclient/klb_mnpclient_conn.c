...
#include "klbnet/klbmnp/klb_mnpclient_conn.h"
...
static int klb_mnpclient_conn_init(klb_netconn_t* p_conn)
{
    ...
    p_conn->vtable.ioctrl = klb_mnpclient_conn_ioctrl; // 对连接进行控制操作: get/set,etc.
    p_conn->vtable.send_normal = klb_mnpclient_conn_send_normal; // 调用者 发送常规数据包
    p_conn->vtable.send_media = klb_mnpclient_conn_send_media; // 调用者 发送媒体数据包
    p_conn->vtable.on_send = klb_mnpclient_conn_on_send; // 当网络上可以发送数据时
    p_conn->vtable.on_recv = klb_mnpclient_conn_on_recv; // 当网络上可以接收数据时
    p_conn->vtable.on_msg = klb_mnpclient_conn_on_msg; // 当网络上有消息传来时
