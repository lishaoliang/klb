...
/// @struct klb_ioselect_closing_t
/// @brief  待关闭的 连接
typedef struct klb_ioselect_closing_t_
{
    klb_netconn_t*          p_netconn;                  ///< 连接
    char                    name[KLB_NETCONN_NAME_MAX]; ///< 名称
}klb_ioselect_closing_t;


/// @struct klb_ioselect_t
/// @brief  多路复用(klb net multiplex)
typedef struct klb_ioselect_t_
{
    klb_hlist_t*            p_conn_hlist;       ///< 当前的所有的连接列表; 存储 klb_netconn_t*
    klb_nlist_t*            p_closing_nlist;    ///< 待移除的 连接列表; 存储 klb_ioselect_closing_t*
...
//////////////////////////////////////////////////////////////////////////
static int klb_ioselect_loop_once_do_closing(klb_netmulti_t* p_multi);


/// @brief 创建
static klb_netmulti_t* klb_ioselect_create(int64_t tc)
{
    klb_netmulti_t* p_multi = KLB_MALLOCZ(klb_netmulti_t, 1, sizeof(klb_ioselect_t));
    klb_ioselect_t* p_io = (klb_ioselect_t*)p_multi->extra;

    p_io->p_conn_hlist = klb_hlist_create(0);
    p_io->p_closing_nlist = klb_nlist_create();

    p_io->now_tc = tc;       ///< 计时: 当前时间
    p_io->last_tc = tc;
...
/// @brief 销毁
static void klb_ioselect_destroy(klb_netmulti_t* p_multi)
{
    klb_ioselect_t* p_io = (klb_ioselect_t*)p_multi->extra;

    // 关闭连接
    klb_ioselect_loop_once_do_closing(p_multi);
...
/// @brief 放入连接对象
static int klb_ioselect_push(klb_netmulti_t* p_multi, klb_netconn_t* p_conn)
{
    klb_ioselect_t* p_io = (klb_ioselect_t*)p_multi->extra;
    ...
    while (true)
    {
        ...
        if (NULL != klb_hlist_push_tail(p_io->p_conn_hlist, name, name_len, p_conn))
        {
            // 成功, 修改名称
...
static klb_netconn_t* klb_ioselect_remove_by_name(klb_netmulti_t* p_multi, const char* p_name)
{
    klb_ioselect_t* p_io = (klb_ioselect_t*)p_multi->extra;

    int name_len = strlen(p_name);
    klb_netconn_t* p_conn = (klb_netconn_t*)klb_hlist_remove_bykey(p_io->p_conn_hlist, p_name, name_len);
...
/// @brief 移除连接对象
/// @return int 0.成功; 非0. 失败
static int klb_ioselect_remove(klb_netmulti_t* p_multi, klb_netconn_t* p_conn)
{
    klb_ioselect_t* p_io = (klb_ioselect_t*)p_multi->extra;

    const char* p_name = klb_netconn_get_name(p_conn);

    klb_netconn_t* p_tmp = klb_ioselect_remove_by_name(p_multi, p_name);
...
/// @brief 托管关闭连接
static int klb_ioselect_closing(klb_netmulti_t* p_multi, klb_netconn_t* p_conn)
{
    klb_ioselect_t* p_io = (klb_ioselect_t*)p_multi->extra;
    ...
    // 放入待关闭 列表
    {
        klb_ioselect_closing_t* p_closing = KLB_MALLOCZ(klb_ioselect_closing_t, 1, 0);
...
static int klb_ioselect_loop_once_do(klb_netmulti_t* p_multi, int64_t now)
{
    klb_ioselect_t* p_io = (klb_ioselect_t*)p_multi->extra;
    ...
    while (NULL != p_iter)
    {
        ...
        if (KLB_SOCKET_OK == p_socket->status)
        {
            ...
            // 非阻塞 connect 完成走 writefds; 仅 reading 时也要监听可写
    ...
    if (0 < ret)
    {
        ...
        while (NULL != p_iter)
        {
            ...
            if (KLB_SOCKET_OK == p_socket->status)
            {
                ...
                else if (0 == p_socket->connected && 0 != p_socket->reading && FD_ISSET(p_socket->fd, &w_fds))
                {
                    klb_socket_set_connected(p_socket, true);
...
static int klb_ioselect_loop_once_do_closing(klb_netmulti_t* p_multi)
{
    ...
    {
        klb_ioselect_closing_t* p_remove = klb_nlist_pop_head(p_io->p_closing_nlist);
        klb_netconn_t* p_conn = klb_ioselect_remove_by_name(p_multi, p_remove->name);
...
static int klb_ioselect_loop_once_onticker(klb_netmulti_t* p_multi, int64_t now)
{
    klb_ioselect_t* p_io = (klb_ioselect_t*)p_multi->extra;
...
/// @brief 调用一次; 需要定期调用
static int klb_ioselect_loop_once(klb_netmulti_t* p_multi, int64_t now)
{
    klb_ioselect_t* p_io = (klb_ioselect_t*)p_multi->extra;
    ...
    // 执行主体业务
    int ret = klb_ioselect_loop_once_do(p_multi, now);

    // 关闭连接; 执行 klb_ioselect_loop_once_do 之后, 也可能有连接被标记为 关闭
    // 这里再次 尝试关闭连接
    klb_ioselect_loop_once_do_closing(p_multi);
    ...
    {
        klb_ioselect_loop_once_onticker(p_multi, now);
        p_io->last_tc = now;
    }
...
klb_iopoll_ops_t g_klb_iopoll_ops = {
    .create = klb_ioselect_create,
    .destroy = klb_ioselect_destroy,

    .push = klb_ioselect_push,

    .remove = klb_ioselect_remove,
    .remove_by_name = klb_ioselect_remove_by_name,
