...
//////////////////////////////////////////////////////////////////////////
// mnp
#define KLUA_KMNP_HANDLE            "KLUA_KMNP_HANDLE*"     ///< Lua meta标示


typedef struct klua_kmnp_t_ klua_kmnp_t;


// 参数
typedef struct klua_kmnp_param_t_
{
...
// C
typedef struct klua_kmnp_inter_t_
{
    klua_kmnp_param_t           param;          ///< 参数

    klua_kmnp_t*                p_ktcp;         ///< ktcp
...
typedef struct klua_kmnp_t_
{
    ...
    klua_kmnp_inter_t*          p_inter;        ///< C内部实现
}klua_kmnp_t;


//////////////////////////////////////////////////////////////////////////

static int call_lua_co_recv_klua_kmnp(klua_kmnp_t* p_kmnp, const char* p_type, klb_buf_t* p_buf)
{
    assert(NULL != p_kmnp);
...
static void free_klua_kmnp_inter(klua_kmnp_inter_t* p_inter)
{
    // 清空
...
static int cb_remove_klua_kmnp(void* p_lparam, void* p_wparam, int id)
{
    //klua_ktcp_t* p_kmnp = (klua_ktcp_t*)p_lparam;
    klua_kmnp_inter_t* p_inter = (klua_kmnp_inter_t*)p_wparam;

    // 可以被移除时, 与lua环境的 gc 没有必然的次序关系
    // 所以 1.与lua 关联的在 gc 或 gc 之前释放; 2. C部分在这里释放
    KLB_FREE_BY(p_inter, free_klua_kmnp_inter);

    return 0;
}

static int klua_kmnp_parse(klua_kmnp_t* p_kmnp, klua_kmnp_inter_t* p_inter, klb_socket_t* p_socket)
{
...
static int cb_recv_klua_kmnp(void* p_lparam, void* p_wparam, int id, int64_t now)
{
    klua_kmnp_t* p_kmnp = (klua_kmnp_t*)p_lparam;
    klua_kmnp_inter_t* p_inter = (klua_kmnp_inter_t*)p_wparam;
    ...
    while (true)
    {
        ...
        if (0 < r)
        {
            ...
            while (true)
            {
                if (0 != klua_kmnp_parse(p_kmnp, p_inter, p_socket))
                {
                    break;
                }
            }
...
static int cb_send_klua_kmnp(void* p_lparam, void* p_wparam, int id, int64_t now)
{
    klua_kmnp_t* p_kmnp = (klua_kmnp_t*)p_lparam;
    klua_kmnp_inter_t* p_inter = (klua_kmnp_inter_t*)p_wparam;
...
static int cb_timer_klua_kmnp(void* p_lparam, void* p_wparam, int id, int64_t now)
{
    klua_kmnp_t* p_kmnp = (klua_kmnp_t*)p_lparam;
    klua_kmnp_inter_t* p_inter = (klua_kmnp_inter_t*)p_wparam;
...
{
    klua_kmnp_t* p_kmnp = (klua_kmnp_t*)lua_newuserdata(L, sizeof(klua_kmnp_t));
    KLB_MEMSET(p_kmnp, 0, sizeof(klua_kmnp_t));
...
{
    klua_kmnp_t* p_kmnp = (klua_kmnp_t*)luaL_checkudata(L, index, KLUA_KMNP_HANDLE);
...
static int klua_kmnp_send_text(lua_State* L)
{
    ...
    klb_buf_t* p_buf = klb_mnp_pack_text(0, 0, (const uint8_t*)p_head, head_len, (const uint8_t*)p_body, body_len);
...
static void klua_kmnp_createmeta(lua_State* L)
{
    static luaL_Reg meth[] = {
        ...
        { "send_media",     klua_kmnp_send_media },
    ...
    luaL_newmetatable(L, KLUA_KMNP_HANDLE);/* metatable for KLUA_KUDP_HANDLE handles */
...
#define KLUA_KMNP_LISTEN_HANDLE    "KLUA_KMNP_LISTEN_HANDLE*"

typedef struct klua_kmnp_listen_param_t_
{
    int max_connect;                            ///< 操作系统监听socket, 能缓存的最大连接数
}klua_kmnp_listen_param_t;

typedef struct klua_kmnp_listen_t_
{
    klua_kmnp_listen_param_t    param;          ///< 参数
...
static void klua_kmnp_listen_createmeta(lua_State* L)
{
    ...
    luaL_newmetatable(L, KLUA_KMNP_LISTEN_HANDLE);/* metatable for KLUA_KTCP_LISTEN_HANDLE handles */
    luaL_setfuncs(L, metameth, 0);          /* add metamethods to new metatable */
    luaL_newlibtable(L, meth);              /* create method table */
    luaL_setfuncs(L, meth, 0);              /* add file methods to method table */
    lua_setfield(L, -2, "__index");         /* metatable.__index = method table */
...
#define KLUA_KMNP_RBUF_MAX          (1024 * 8)              ///< 临时读取缓存最大值
#define KLUA_KMNP_READ_MAX          (1024 * 512)            ///< 缓存最大值后,暂停读取

static void default_param_klua_kmnp(klua_kmnp_param_t* p_param)
{
    p_param->rbuf_max = KLUA_KMNP_RBUF_MAX; // 临时读取缓存最大值
    p_param->read_max = KLUA_KMNP_READ_MAX; // 缓存最大值后,暂停读取
    p_param->tls = false;
}

static void check_param_klua_kmnp(lua_State* L, int from, klua_kmnp_param_t* p_param)
{
...
klua_kmnp_t* new_connect_klua_kmnp(lua_State* L, klb_socket_fd fd, klua_kmnp_param_t* p_param)
{
    ...
    klua_kmnp_inter_t* p_inter = KLB_MALLOCZ(klua_kmnp_inter_t, 1, 0);
    klua_kmnp_t* p_kmnp = new_klua_kmnp(L);
    p_kmnp->L = L;
    p_kmnp->p_env = klua_env_get_by_L(L);
    p_kmnp->p_multi = klua_ex_multiplex_get_by_L(L);
    p_kmnp->p_inter = p_inter;

    memcpy(&p_inter->param, p_param, sizeof(klua_kmnp_param_t));
    ...
    o.cb_remove = cb_remove_klua_kmnp;
    o.cb_recv = cb_recv_klua_kmnp;
    o.cb_send = cb_send_klua_kmnp;
    o.cb_timer = cb_timer_klua_kmnp;
    o.p_lparam = p_kmnp;
    o.p_wparam = p_inter;
...
static int lib_klua_kmnp_connect(lua_State* L)
{
    const char* p_host = luaL_checkstring(L, 1);
    lua_Integer port = luaL_checkinteger(L, 2);

    // param
    klua_kmnp_param_t param = { 0 };
    default_param_klua_kmnp(&param);
    check_param_klua_kmnp(L, 2, &param);
    ...
    // new connect
    new_connect_klua_kmnp(L, fd, &param);
    return 1;
}
...
static int on_accept_klua_kmnp_listen(void* ptr, klb_socket_fd fd, const struct sockaddr_in* p_addr, bool tls, const klb_socket_tls_param_t* p_tls_param)
{
    ...
    if (p_listen->co_accept)
    {
        ...
        if (NULL != L)
        {
            ...
            new_connect_klua_kmnp(L, fd, &param);               /* 1st argument */
...
static int lib_klua_kmnp_listen(lua_State* L)
{
    ...
    klb_listen_set_accept(p_listen->p_listen, on_accept_klua_kmnp_listen, p_listen);
...
int klua_open_kmnp(lua_State* L)
{
    static luaL_Reg lib[] =
    {
        { "connect",    lib_klua_kmnp_connect },

        { "listen",     lib_klua_kmnp_listen },
