...
static int call_co_recv_text_klua_ksmpclientrpc(klua_ksmpclientrpc_t* p_client)
{
        ...
        (0 < klb_nlist_size(p_client->p_text_nlist) || 0 < klb_nlist_size(p_client->p_media_nlist)))
    {
        ...
        // Bug. 当调用 lua_pcall 函数之后, 函数执行到 Lua 层
        // 在 Lua 可能会依然调用 co_recv(klua_ksmpclientrpc_co_recv) 函数; 这里会存在执行函数的交替执行
...
static int klua_ksmpclientrpc_send_text(lua_State* L)
{
    ...
    if (NULL != p_client->p_rpc_conn)
    {
        ret = klb_netconn_send_text(p_client->p_rpc_conn, 0, 0, (const uint8_t*)p_head, head_len, NULL, 0);
    }
...
static void klua_ksmpclientrpc_createmeta(lua_State* L)
{
    static luaL_Reg meth[] = {
        ...
        { "send_text",      klua_ksmpclientrpc_send_text },

        { "co_recv",        klua_ksmpclientrpc_co_recv },
...
static int klua_ksmpclient_connect_rpc(lua_State* L)
{
    ...
    klb_netconn_t* p_smp_conn = klb_smpclientrpc_connect(p_netmulti, p_host, port);
    ...
    {
        // 
        klua_ksmpclientrpc_t* p_client = new_klua_ksmpclientrpc(L);       ///< #1. meta: klua_ksmpclient_createmeta
        ...
        p_client->p_netmulti = p_netmulti;
        p_client->p_rpc_conn = p_smp_conn;
        ...
        // 绑定数据接收
        p_smp_conn->p_udata = p_client;
        klb_netconn_bind_recv_data(p_smp_conn, on_recv_data_klua_ksmpclientrpc);
    }
