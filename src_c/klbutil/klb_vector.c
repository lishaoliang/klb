...
/// @def   KLB_VECTOR_MIN
/// @brief 容量/掩码表序号的最小值
#define KLB_VECTOR_MIN  6       // 2^6 = 64


/// @def   KLB_HMAP_IDX_TABLE_MAX
/// @brief 容量/掩码表序号的最大值
#define KLB_VECTOR_MAX  28      // 2^28 * sizeof(char*) <= 2^32


/// @struct klb_vector_talbe_t
/// @brief  容量/掩码表
typedef struct klb_vector_talbe_t_
{
    uint32_t    max;            ///< 容量最大值
    uint32_t    mask;           ///< 掩码值
}klb_vector_talbe_t;
...
///< 容量/掩码表
static klb_vector_talbe_t s_klb_vector_talbe[32] = {
...
{
    klb_vector_t* p_vector = KLB_MALLOCZ(klb_vector_t, 1, 0);

    p_vector->idx_talbe = KLB_VECTOR_MIN;

    // 常驻内存量
    p_vector->p_idx = KLB_MALLOC(char*, s_klb_vector_talbe[p_vector->idx_talbe].max, 0);
...
static void klb_vector_check_resize_inc(klb_vector_t* p_vector)
{
    // 检查扩大容量
    // 条件: 当前容量的1/2 <= 当前节点数目; 没到最大值KLB_VECTOR_MAX
    if (KLB_VECTOR_MAX <= p_vector->idx_talbe)
    {
        return; // 已经最大容量了
    }

    assert(KLB_VECTOR_MIN <= p_vector->idx_talbe);
    if (p_vector->size < s_klb_vector_talbe[p_vector->idx_talbe - 1].max)
    {
        return; // 在适合的范围内, 无需扩容
    }
    ...
    char** ptr = KLB_REALLOC(p_vector->p_idx, char*, s_klb_vector_talbe[p_vector->idx_talbe].max, 0);
    assert(NULL != ptr);

    p_vector->p_idx = ptr;
}

static void klb_vector_check_resize_dec(klb_vector_t* p_vector)
{
    // 检查缩小容量
    // 条件: 当前容量的1/4 >= 当前节点数目; 没有到最小值KLB_VECTOR_MIN
    if (p_vector->idx_talbe <= KLB_VECTOR_MIN)
    {
        return; // 已经最小了
    }

    if (s_klb_vector_talbe[p_vector->idx_talbe - 2].max < p_vector->size)
    {
...
//{
//    klb_vector_check_resize_inc(p_vector);
//
//    // move
//}

int klb_vector_push_tail(klb_vector_t* p_vector, void* p_data)
{
    klb_vector_check_resize_inc(p_vector);

    int idx = p_vector->size;
    p_vector->p_idx[idx] = p_data;
    p_vector->size++;
...
void* klb_vector_pop_head(klb_vector_t* p_vector)
{
    ...
    klb_vector_check_resize_dec(p_vector);
    return ptr;
}
...
void* klb_vector_update(klb_vector_t* p_vector, int index, void* p_data)
{
    if (0 <= index && index < p_vector->size)
    {
        void* ptr = p_vector->p_idx[index];
        p_vector->p_idx[index] = p_data;
