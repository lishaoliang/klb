...
//////////////////////////////////////////////////////////////////////////
// 前置定义
static void klbwnd_combomenu_item_quit_attribute(klbwnd_combomenu_item_t* p_item);


//////////////////////////////////////////////////////////////////////////
// 继承重写方法

static void klbwnd_combomenu_item_destroy(klb_wnd_t* p_wnd)
{
    klbwnd_combomenu_item_t* p_item = (klbwnd_combomenu_item_t*)p_wnd->ctrl;

    klbwnd_combomenu_item_quit(p_wnd);

    KLB_FREE(p_wnd);
}

static void klbwnd_combomenu_item_on_paint_status(klb_wnd_t* p_wnd, klbwnd_combomenu_item_t* p_item, klbwnd_combomenu_item_css_t* p_css, klbuicssex_attributes_t* p_attr, klb_rect_t* p_rect)
{
    ...
    // 标题文本
    klbuicssex_draw_text(p_wnd, p_item->title, p_rect, &p_attr->border, &p_css->padding, &p_attr->text, &p_attr->font);
}

static int klbwnd_combomenu_item_on_paint(klb_wnd_t* p_wnd)
{
    klbwnd_combomenu_item_t* p_item = (klbwnd_combomenu_item_t*)p_wnd->ctrl;
    ...
    klbwnd_combomenu_item_css_t* p_css = p_item->p_css;
    ...
    {
        klbwnd_combomenu_item_on_paint_status(p_wnd, p_item, p_css, &p_css->disable, &paint_rect);
    }
    else if (KLB_WND_STATUS_FOCUS & p_wnd->state.status)
    {
        klbwnd_combomenu_item_on_paint_status(p_wnd, p_item, p_css, &p_css->focus, &paint_rect);
    }
    else
    {
        klbwnd_combomenu_item_on_paint_status(p_wnd, p_item, p_css, &p_css->normal, &paint_rect);
    }
...
static int klbwnd_combomenu_item_on_control(klb_wnd_t* p_wnd, int msg, const klb_point_t* p_pt1, const klb_point_t* p_pt2, int lparam, int wparam)
{
    klbwnd_combomenu_item_t* p_item = (klbwnd_combomenu_item_t*)p_wnd->ctrl;
    ...
    {
    case KLBUI_onpaint:
        return klbwnd_combomenu_item_on_paint(p_wnd);
        break;
    default:
        break;
    }
...
static void klbwnd_combomenu_item_init_attribute(klb_wnd_t* p_wnd, klbwnd_combomenu_item_t* p_item)
{
...
void klbwnd_combomenu_item_init(klb_wnd_t* p_wnd, klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    p_wnd->vtable.on_control = klbwnd_combomenu_item_on_control;    // 这里指定
    ...
    // 初始化内部变量
    klbwnd_combomenu_item_init_attribute(p_wnd, p_item);
}
...
void klbwnd_combomenu_item_quit(klb_wnd_t* p_wnd)
{
    ...
    klbwnd_combomenu_item_quit_attribute(p_item);
}
...
{
    klb_wnd_t* p_wnd = KLB_MALLOCZ(klb_wnd_t, 1, sizeof(klbwnd_combomenu_item_t));

    klbwnd_combomenu_item_init(p_wnd, p_gui, x, y, w, h);

    // 补写 destroy 函数
    p_wnd->vtable.destroy = klbwnd_combomenu_item_destroy;
