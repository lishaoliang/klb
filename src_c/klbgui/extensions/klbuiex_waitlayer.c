...
//////////////////////////////////////////////////////////////////////////
// 前置定义
static void clear_ticker_list_klbuiex_waitlayer(klbuiex_waitlayer_t* p_ex);
static int do_ticker_list_klbuiex_waitlayer(klbuiex_waitlayer_t* p_ex, klb_nlist_t* p_list);
...
static int klbuiex_waitlayer_control(void* ptr, klb_gui_t* p_gui, int msg, uint8_t* p_param_in_out, int param_size)
{
    klbuiex_waitlayer_t* p_ex = (klbuiex_waitlayer_t*)ptr;

    if (KLBUI_EX_MSG_quit == msg)
    {
        klbuiex_waitlayer_bind_wnd(p_ex, NULL); // 解绑窗口
    }
...
static int klbuiex_waitlayer_loop_once(void* ptr, klb_gui_t* p_gui, int64_t now)
{
    klbuiex_waitlayer_t* p_ex = (klbuiex_waitlayer_t*)ptr;

    if (!klbuiex_waitlayer_is_show(p_ex))
    {
        return 0; // 未显示  无需处理
    }
    ...
    // 处理窗口定时器
    do_ticker_list_klbuiex_waitlayer(p_ex, p_ex->p_ticker_list);
...
// 将需要处理定时器的 窗口加入链表
static int do_push_ticker_list_klbuiex_waitlayer(klbuiex_waitlayer_t* p_ex, klb_nlist_t* p_list, klb_wnd_t* p_wnd)
{
    // 递归 遍历窗口
    if (NULL == p_wnd)
    {
    ...
    while (NULL != p_next)
    {
        do_push_ticker_list_klbuiex_waitlayer(p_ex, p_list, p_next);
...
static void do_control_event_recursive_klbuiex_waitlayer(klb_wnd_t* p_wnd, int e, const klb_point_t* p_pt1, const klb_point_t* p_pt2, int lparam, int wparam)
{
    if (NULL == p_wnd)
    {
        return;
    ...
    while (NULL != p_next)
    {
        do_control_event_recursive_klbuiex_waitlayer(p_next, e, p_pt1, p_pt2, lparam, wparam);
...
// 窗口 绑定时 的处理
static void do_wnd_klbuiex_waitlayer(klbuiex_waitlayer_t* p_ex)
{
    klb_wnd_t* p_wnd = p_ex->p_wait_wnd;

    // "onload" 事件
    {
        // on_control 递归调用所有的控件
        do_control_event_recursive_klbuiex_waitlayer(p_wnd, KLBUI_onload, NULL, NULL, 0, 0);
    ...
    // "onpredraw" 事件
    {
        ...
        // on_control 递归调用所有的控件
        do_control_event_recursive_klbuiex_waitlayer(p_wnd, KLBUI_onpredraw, NULL, NULL, 0, 0);
    }
    ...
    do_push_ticker_list_klbuiex_waitlayer(p_ex, p_ex->p_ticker_list, p_wnd);
...
// 窗口 解绑 时的 处理
static void do_wnd_end_klbuiex_waitlayer(klbuiex_waitlayer_t* p_ex)
{
    klb_wnd_t* p_wnd = p_ex->p_wait_wnd;
    ...
    // "onunload" 事件
    {
        ...
        // on_control 递归调用所有的控件
        do_control_event_recursive_klbuiex_waitlayer(p_wnd, KLBUI_onunload, NULL, NULL, 0, 0);
    }

    // 清理定时器列表
    clear_ticker_list_klbuiex_waitlayer(p_ex);
...
void klbuiex_waitlayer_bind_wnd(klbuiex_waitlayer_t* p_ex, klb_wnd_t* p_top)
{
    ...
    {
        // 窗口结束 处理 
        do_wnd_end_klbuiex_waitlayer(p_ex);
...
bool klbuiex_waitlayer_wnd_in_bind(klbuiex_waitlayer_t* p_ex, klb_wnd_t* p_wnd)
{
    ...
    if (NULL != p_ex->p_wait_wnd && p_top == p_ex->p_wait_wnd)
    {
        return true;
    }
