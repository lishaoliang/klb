...
static void klbwnd_time_on_paint_status(klb_wnd_t* p_wnd, klbwnd_time_t* p_time, klbwnd_time_css_t* p_css, klbuicssex_attributes_t* p_attr, klb_rect_t* p_rect)
{
    ...
    // 标题文本
    klbuicssex_draw_text(p_wnd, p_time->title, p_rect, &p_attr->border, &p_css->padding, &p_attr->text, &p_attr->font);
}
...
void klbwnd_time_set_title(klb_wnd_t* p_wnd, const char* p_title)
{
    klbwnd_time_t* p_time = (klbwnd_time_t*)p_wnd->ctrl;

    p_time->title = sdscpy(p_time->title, p_title);
}

const sds klbwnd_time_get_title(klb_wnd_t* p_wnd)
{
    klbwnd_time_t* p_time = (klbwnd_time_t*)p_wnd->ctrl;
...
void klbwnd_time_set_value(klb_wnd_t* p_wnd, const char* p_value)
{
    klbwnd_time_t* p_time = (klbwnd_time_t*)p_wnd->ctrl;

    p_time->value = sdscpy(p_time->value, p_value);
}

const sds klbwnd_time_get_value(klb_wnd_t* p_wnd)
{
    klbwnd_time_t* p_time = (klbwnd_time_t*)p_wnd->ctrl;
...
static void klbwnd_time_init_attribute(klbwnd_time_t* p_time)
{
    p_time->title = sdsempty();
    p_time->value = sdsempty();
}

static void klbwnd_time_quit_attribute(klbwnd_time_t* p_time)
{
    KLB_FREE_BY(p_time->title, sdsfree);
    KLB_FREE_BY(p_time->value, sdsfree);
}
...
void klbwnd_time_init(klb_wnd_t* p_wnd, klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    // 
    klbwnd_time_init_attribute(p_time);
}
