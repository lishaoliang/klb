...
#include "klbgui/wnd/klbwnd_calendar_days.h"
...
static void klbwnd_calendar_days_on_paint_status(klb_wnd_t* p_wnd, klbwnd_calendar_days_t* p_days, klbwnd_calendar_days_css_t* p_css, klbuicssex_attributes_t* p_attr, klb_rect_t* p_rect)
{
    ...
    // 标题文本
    klbuicssex_draw_text(p_wnd, p_days->title, p_rect, &p_attr->border, &p_css->padding, &p_attr->text, &p_attr->font);
}
...
static int klbwnd_calendar_days_on_paint(klb_wnd_t* p_wnd)
{
    ...
    klbwnd_calendar_days_css_t* p_css = p_days->p_css;
    ...
    {
        klbwnd_calendar_days_on_paint_status(p_wnd, p_days, p_css, &p_css->disable, &paint_rect);
    }
    else if (KLB_WND_STATUS_FOCUS & p_wnd->state.status)
    {
        klbwnd_calendar_days_on_paint_status(p_wnd, p_days, p_css, &p_css->focus, &paint_rect);
    }
    else
    {
        klbwnd_calendar_days_on_paint_status(p_wnd, p_days, p_css, &p_css->normal, &paint_rect);
    }
...
void klbwnd_calendar_days_set_css(klb_wnd_t* p_wnd, klbwnd_calendar_days_css_t* p_css)
{
    klbwnd_calendar_days_t* p_days = (klbwnd_calendar_days_t*)p_wnd->ctrl;
...
void klbwnd_calendar_days_set_title(klb_wnd_t* p_wnd, const char* p_title)
{
    klbwnd_calendar_days_t* p_days = (klbwnd_calendar_days_t*)p_wnd->ctrl;

    p_days->title = sdscpy(p_days->title, p_title);
}

const sds klbwnd_calendar_days_get_title(klb_wnd_t* p_wnd)
{
    klbwnd_calendar_days_t* p_days = (klbwnd_calendar_days_t*)p_wnd->ctrl;
...
void klbwnd_calendar_days_set_value(klb_wnd_t* p_wnd, const char* p_value)
{
    klbwnd_calendar_days_t* p_days = (klbwnd_calendar_days_t*)p_wnd->ctrl;

    p_days->value = sdscpy(p_days->value, p_value);
}

const sds klbwnd_calendar_days_get_value(klb_wnd_t* p_wnd)
{
    klbwnd_calendar_days_t* p_days = (klbwnd_calendar_days_t*)p_wnd->ctrl;
...
{
    p_days->title = sdsempty();
    p_days->value = sdsempty();
}

static void klbwnd_calendar_days_quit_attribute(klbwnd_calendar_days_t* p_days)
{
    KLB_FREE_BY(p_days->title, sdsfree);
    KLB_FREE_BY(p_days->value, sdsfree);
}

//////////////////////////////////////////////////////////////////////////
// css init / quit

void klbwnd_calendar_days_css_init(klbwnd_calendar_days_css_t* p_css, klb_gui_t* p_gui)
{
...
void klbwnd_calendar_days_css_quit(klbwnd_calendar_days_css_t* p_css)
{
