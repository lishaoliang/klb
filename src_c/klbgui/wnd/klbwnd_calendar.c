...
//////////////////////////////////////////////////////////////////////////
// 前置定义
static void klbwnd_calendar_quit_attribute(klbwnd_calendar_t* p_cal);
...
static void klbwnd_calendar_destroy(klb_wnd_t* p_wnd)
{
    ...
    klbwnd_calendar_quit_attribute(p_cal);

    KLB_FREE(p_wnd);
}

static void klbwnd_calendar_on_paint_status(klb_wnd_t* p_wnd, klbwnd_calendar_t* p_cal, klbwnd_calendar_css_t* p_css, klbuicssex_attributes_t* p_attr, klb_rect_t* p_rect)
{
    ...
    // 标题文本
    klbuicssex_draw_text(p_wnd, p_cal->title, p_rect, &p_attr->border, &p_css->padding, &p_attr->text, &p_attr->font);
}
...
void klbwnd_calendar_set_title(klb_wnd_t* p_wnd, const char* p_title)
{
    klbwnd_calendar_t* p_cal = (klbwnd_calendar_t*)p_wnd->ctrl;

    p_cal->title = sdscpy(p_cal->title, p_title);
}

const sds klbwnd_calendar_get_title(klb_wnd_t* p_wnd)
{
    klbwnd_calendar_t* p_cal = (klbwnd_calendar_t*)p_wnd->ctrl;
...
void klbwnd_calendar_set_value(klb_wnd_t* p_wnd, const char* p_value)
{
    klbwnd_calendar_t* p_cal = (klbwnd_calendar_t*)p_wnd->ctrl;

    p_cal->value = sdscpy(p_cal->value, p_value);
}

const sds klbwnd_calendar_get_value(klb_wnd_t* p_wnd)
{
    klbwnd_calendar_t* p_cal = (klbwnd_calendar_t*)p_wnd->ctrl;
...
{
    p_cal->title = sdsempty();
    p_cal->value = sdsempty();
}

static void klbwnd_calendar_quit_attribute(klbwnd_calendar_t* p_cal)
{
    KLB_FREE_BY(p_cal->title, sdsfree);
    KLB_FREE_BY(p_cal->value, sdsfree);
}
...
void klbwnd_calendar_quit(klb_wnd_t* p_wnd)
{
    klbwnd_calendar_t* p_cal = (klbwnd_calendar_t*)p_wnd->ctrl;
