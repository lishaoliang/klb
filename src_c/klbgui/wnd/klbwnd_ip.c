...
//////////////////////////////////////////////////////////////////////////
// 前置定义
static void klbwnd_ip_quit_attribute(klbwnd_ip_t* p_ip);
...
static void klbwnd_ip_on_paint_status(klb_wnd_t* p_wnd, klbwnd_ip_t* p_ip, klbwnd_ip_css_t* p_css, klbuicssex_attributes_t* p_attr, klb_rect_t* p_rect)
{
    ...
    // 标题文本
    klbuicssex_draw_text(p_wnd, p_ip->title, p_rect, &p_attr->border, &p_css->padding, &p_attr->text, &p_attr->font);
}
...
void klbwnd_ip_set_title(klb_wnd_t* p_wnd, const char* p_title)
{
    ...
    p_ip->title = sdscpy(p_ip->title, p_title);
}

const sds klbwnd_ip_get_title(klb_wnd_t* p_wnd)
{
    klbwnd_ip_t* p_ip = (klbwnd_ip_t*)p_wnd->ctrl;
...
void klbwnd_ip_set_value(klb_wnd_t* p_wnd, const char* p_value)
{
    ...
    p_ip->value = sdscpy(p_ip->value, p_value);
}
...
static void klbwnd_ip_quit_attribute(klbwnd_ip_t* p_ip)
{
    KLB_FREE_BY(p_ip->title, sdsfree);
...
void klbwnd_ip_css_quit(klbwnd_ip_css_t* p_css)
{
...
void klbwnd_ip_init(klb_wnd_t* p_wnd, klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    p_wnd->vtable.on_control = klbwnd_ip_on_control;    // 这里指定
...
void klbwnd_ip_quit(klb_wnd_t* p_wnd)
{
    klbwnd_ip_t* p_ip = (klbwnd_ip_t*)p_wnd->ctrl;

    klbwnd_ip_quit_attribute(p_ip);
}
