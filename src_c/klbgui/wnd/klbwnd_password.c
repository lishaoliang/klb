...
//////////////////////////////////////////////////////////////////////////
// 前置定义
static void klbwnd_password_quit_attribute(klbwnd_password_t* p_passwd);
...
static void klbwnd_password_on_paint_status(klb_wnd_t* p_wnd, klbwnd_password_t* p_passwd, klbwnd_password_css_t* p_css, klbuicssex_attributes_t* p_attr, klb_rect_t* p_rect)
{
    ...
    // 标题文本
    klbuicssex_draw_text(p_wnd, p_passwd->title, p_rect, &p_attr->border, &p_css->padding, &p_attr->text, &p_attr->font);
}
...
static int klbwnd_password_on_paint(klb_wnd_t* p_wnd)
{
    ...
    {
        klbwnd_password_on_paint_status(p_wnd, p_passwd, p_css, &p_css->disable, &paint_rect);
    }
    else if (KLB_WND_STATUS_FOCUS & p_wnd->state.status)
    {
        klbwnd_password_on_paint_status(p_wnd, p_passwd, p_css, &p_css->focus, &paint_rect);
    }
    else
    {
        klbwnd_password_on_paint_status(p_wnd, p_passwd, p_css, &p_css->normal, &paint_rect);
    }
...
void klbwnd_password_set_title(klb_wnd_t* p_wnd, const char* p_title)
{
    ...
    p_passwd->title = sdscpy(p_passwd->title, p_title);
}
...
void klbwnd_password_set_value(klb_wnd_t* p_wnd, const char* p_value)
{
    klbwnd_password_t* p_passwd = (klbwnd_password_t*)p_wnd->ctrl;

    p_passwd->value = sdscpy(p_passwd->value, p_value);
}

const sds klbwnd_password_get_value(klb_wnd_t* p_wnd)
{
    klbwnd_password_t* p_passwd = (klbwnd_password_t*)p_wnd->ctrl;

    return p_passwd->value;
}
...
{
    p_passwd->index = 0;
    p_passwd->title = sdsempty();
    p_passwd->value = sdsempty();
}

static void klbwnd_password_quit_attribute(klbwnd_password_t* p_passwd)
{
    KLB_FREE_BY(p_passwd->title, sdsfree);
    KLB_FREE_BY(p_passwd->value, sdsfree);
}
...
void klbwnd_password_css_quit(klbwnd_password_css_t* p_css)
{
...
void klbwnd_password_init(klb_wnd_t* p_wnd, klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    p_wnd->vtable.on_control = klbwnd_password_on_control;  // 这里指定
...
void klbwnd_password_quit(klb_wnd_t* p_wnd)
{
    klbwnd_password_t* p_passwd = (klbwnd_password_t*)p_wnd->ctrl;

    klbwnd_password_quit_attribute(p_passwd);
}
