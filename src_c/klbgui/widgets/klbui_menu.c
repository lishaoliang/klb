...
static void klbui_menu_destroy(klb_wnd_t* p_wnd)
{
    ...
    {
        klbwnd_menu_css_quit(p_menu->p_mycss);
...
{
    klb_map_t* ptr = klb_gui_globalcss_map(p_gui, KLBUI_kmenu);
    if (NULL != ptr)
    {
        return;
    }
    ...
    ptr = klb_gui_new_globalcss_map(p_gui, KLBUI_kmenu);

    // 添加全局 CSS 
    klbwnd_menu_css_t* p_css = KLB_MALLOCZ(klbwnd_menu_css_t, 1, 0);
    klbwnd_menu_css_init(p_css, p_gui);
    klb_gui_globalcss_set_ptr(p_gui, KLBUI_kmenu, p_css, destroy_globalcss_klbwnd_menu);
...
static klb_wnd_t* klbui_menu_create(klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    // step5. 默认使用公用全局CSS属性
    p_menu->p_globalcss = (klbwnd_menu_css_t*)klb_gui_globalcss_get_ptr(p_gui, KLBUI_kmenu);
...
int klbui_register_kmenu(klb_gui_t* p_gui)
{
    ...
    return klb_gui_register(p_gui, KLBUI_kmenu, klbui_menu_create);
}
