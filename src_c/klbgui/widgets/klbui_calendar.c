...
{
    klbui_calendar_t* p_calendar = (klbui_calendar_t*)p_wnd->ctrl;

    KLB_FREE(p_wnd);
}

static int klbui_calendar_on_paint(klb_wnd_t* p_wnd)
{
    klbui_calendar_t* p_calendar = (klbui_calendar_t*)p_wnd->ctrl;
...
static int klbui_calendar_on_control(klb_wnd_t* p_wnd, int msg, const klb_point_t* p_pt1, const klb_point_t* p_pt2, int lparam, int wparam)
{
    klbui_calendar_t* p_calendar = (klbui_calendar_t*)p_wnd->ctrl;
    ...
    {
    case KLB_GUI_PAINT:
        return klbui_calendar_on_paint(p_wnd);
    default:
        break;
    }
...
static int klbui_calendar_on_command(klb_wnd_t* p_wnd, int msg, const klb_point_t* p_pt1, const klb_point_t* p_pt2, int lparam, int wparam)
{
    klbui_calendar_t* p_calendar = (klbui_calendar_t*)p_wnd->ctrl;
...
klb_wnd_t* klbui_calendar_create(klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    p_wnd->vtable.on_control = klbui_calendar_on_control;
    p_wnd->vtable.on_command = klbui_calendar_on_command;
