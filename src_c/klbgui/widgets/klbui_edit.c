...
typedef struct klbui_edit_t_
{
    ...
    klbwnd_edit_css_t*  p_globalcss;    ///< 全局公共的CSS
    klbwnd_edit_css_t*  p_mycss;        ///< 私有的CSS
...
static void klbui_edit_destroy(klb_wnd_t* p_wnd)
{
    ...
    {
        klbwnd_edit_css_quit(p_edit->p_mycss);
...
{
    klb_map_t* ptr = klb_gui_globalcss_map(p_gui, KLBUI_kedit);
    if (NULL != ptr)
    {
        return;
    }
    ...
    ptr = klb_gui_new_globalcss_map(p_gui, KLBUI_kedit);

    // 添加全局 CSS 
    klbwnd_edit_css_t* p_css = KLB_MALLOCZ(klbwnd_edit_css_t, 1, 0);
    klbwnd_edit_css_init(p_css, p_gui);
    klb_gui_globalcss_set_ptr(p_gui, KLBUI_kedit, p_css, destroy_globalcss_klbwnd_edit);
    ...
    KLBUI_GLOBAL_edit_bind("color:disable", globalcss_klbui_edit_text_color_disable);
    ...
    KLBUI_GLOBAL_edit_bind("text-align:disable", globalcss_klbui_edit_text_align_disable);
    ...
    KLBUI_GLOBAL_edit_bind("font-size:disable", globalcss_klbui_edit_font_size_disable);
    ...
    KLBUI_GLOBAL_edit_bind("background-color:disable", globalcss_klbui_edit_background_color_disable);
    ...
    KLBUI_GLOBAL_edit_bind("background-image:disable", globalcss_klbui_edit_background_image_disable);
    ...
    KLBUI_GLOBAL_edit_bind("border-width:disable", globalcss_klbui_edit_border_width_disable);
    ...
    KLBUI_GLOBAL_edit_bind("border-color:disable", globalcss_klbui_edit_border_color_disable);
}
...
static klb_wnd_t* klbui_edit_create(klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    // step5. 默认使用公用全局CSS属性
    p_edit->p_globalcss = (klbwnd_edit_css_t*)klb_gui_globalcss_get_ptr(p_gui, KLBUI_kedit);
...
{
    klbui_edit_init_globalcss(p_gui); // 初始化全局CSS, 需要注册时初始化, 否则全局属性无法生效
    return klb_gui_register(p_gui, KLBUI_kedit, klbui_edit_create);
}
