...
static void klbui_edit_destroy(klb_wnd_t* p_wnd)
{
    ...
    klbui_edit_quit_attribute(p_edit);
    p_edit->p_func_map = NULL;
...
static int klbui_edit_on_paint(klb_wnd_t* p_wnd)
{
    ...
    // 移除外边距
    paint_rect.x += p_edit->margin.left;
    paint_rect.y += p_edit->margin.top;
    paint_rect.w -= (p_edit->margin.left + p_edit->margin.right);
    paint_rect.h -= (p_edit->margin.top + p_edit->margin.bottom);

    if (KLB_WND_STYLE_NOFOCUS & p_wnd->state.style)
    {
        klbui_edit_on_paint_status(p_wnd, p_edit, &p_edit->disable, &paint_rect);
    }
    else if (KLB_WND_STATUS_FOCUS & p_wnd->state.status)
    {
        klbui_edit_on_paint_status(p_wnd, p_edit, &p_edit->focus, &paint_rect);
    }
    else
    {
        klbui_edit_on_paint_status(p_wnd, p_edit, &p_edit->normal, &paint_rect);
    }
