...
#include "klbgui/wnd/klbwnd_password.h"
...
typedef struct klbui_password_t_
{
    ...
    klbwnd_password_css_t*  p_globalcss;    ///< 全局公共的CSS
    klbwnd_password_css_t*  p_mycss;        ///< 私有的CSS
...
static void klbui_password_destroy(klb_wnd_t* p_wnd)
{
    ...
    // 退出基础部分
    klbwnd_password_quit(p_wnd);

    // 退出css
    if (NULL != p_passwd->p_mycss)
    {
        klbwnd_password_css_quit(p_passwd->p_mycss);
...
{
    klbwnd_password_css_t* p_css = (klbwnd_password_css_t*)ptr;
    klbwnd_password_css_quit(p_css);
    KLB_FREE(p_css);
}
...
static void klbui_password_init_globalcss(klb_gui_t* p_gui)
{
    ...
    ptr = klb_gui_new_globalcss_map(p_gui, KLBUI_kpassword);

    // 添加全局 CSS 
    klbwnd_password_css_t* p_css = KLB_MALLOCZ(klbwnd_password_css_t, 1, 0);
    klbwnd_password_css_init(p_css, p_gui);
    klb_gui_globalcss_set_ptr(p_gui, KLBUI_kpassword, p_css, destroy_globalcss_klbwnd_password);
    ...
    KLBUI_GLOBAL_passwd_bind("color:disable", globalcss_klbui_password_text_color_disable);
    ...
    KLBUI_GLOBAL_passwd_bind("text-align:disable", globalcss_klbui_password_text_align_disable);
    ...
    KLBUI_GLOBAL_passwd_bind("font-size:disable", globalcss_klbui_password_font_size_disable);
    ...
    KLBUI_GLOBAL_passwd_bind("background-color:disable", globalcss_klbui_password_background_color_disable);
    ...
    KLBUI_GLOBAL_passwd_bind("background-image:disable", globalcss_klbui_password_background_image_disable);
    ...
    KLBUI_GLOBAL_passwd_bind("border-width:disable", globalcss_klbui_password_border_width_disable);
    ...
    KLBUI_GLOBAL_passwd_bind("border-color:disable", globalcss_klbui_password_border_color_disable);
...
{
    klbuicssex_attribute_sds(&(p_passwd->password.title), p_wnd, method, p_in, p_out);
}

static void on_klbui_password_value(klb_wnd_t* p_wnd, klbui_password_t* p_passwd, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_attribute_sds(&(p_passwd->password.value), p_wnd, method, p_in, p_out);
}
...
{
    klb_map_t* ptr = klb_gui_css_map(p_gui, KLBUI_kpassword);
    if (NULL != ptr)
    {
    ...
    // 未找到, 则新添加 解析map, 及处理函数
    ptr = klb_gui_new_css_map(p_gui, KLBUI_kpassword);
    p_passwd->p_func_map = ptr;
    ...
    KLBUI_passwd_bind("color:disable", on_klbui_password_text_color_disable);
    ...
    KLBUI_passwd_bind("text-align:disable", on_klbui_password_text_align_disable);
    ...
    KLBUI_passwd_bind("font-size:disable", on_klbui_password_font_size_disable);
    ...
    KLBUI_passwd_bind("background-color:disable", on_klbui_password_background_color_disable);
    ...
    KLBUI_passwd_bind("background-image:disable", on_klbui_password_background_image_disable);
    ...
    KLBUI_passwd_bind("border-width:disable", on_klbui_password_border_width_disable);
    ...
    KLBUI_passwd_bind("border-color:disable", on_klbui_password_border_color_disable);
    ...
    KLBUI_passwd_bind("value", on_klbui_password_value);
}
...
{
    klbui_password_init_globalcss(p_gui); // 初始化全局CSS, 需要注册时初始化, 否则全局属性无法生效
