...
static void klbui_tab_destroy(klb_wnd_t* p_wnd)
{
    ...
    {
        klbwnd_tab_css_quit(p_tab->p_mycss);
...
{
    klb_map_t* ptr = klb_gui_globalcss_map(p_gui, KLBUI_ktab);
    if (NULL != ptr)
    {
        return;
    }
    ...
    ptr = klb_gui_new_globalcss_map(p_gui, KLBUI_ktab);

    // 添加全局 CSS 
    klbwnd_tab_css_t* p_css = KLB_MALLOCZ(klbwnd_tab_css_t, 1, 0);
    klbwnd_tab_css_init(p_css, p_gui);
    klb_gui_globalcss_set_ptr(p_gui, KLBUI_ktab, p_css, destroy_globalcss_klbwnd_tab);
...
static klb_wnd_t* klbui_tab_create(klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    // step5. 默认使用公用全局CSS属性
    p_tab->p_globalcss = (klbwnd_tab_css_t*)klb_gui_globalcss_get_ptr(p_gui, KLBUI_ktab);
...
{
    klbui_tab_init_globalcss(p_gui);
    return klb_gui_register(p_gui, KLBUI_ktab, klbui_tab_create);
}
