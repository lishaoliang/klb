...
static int klbui_tab_on_load(klb_wnd_t* p_wnd, klbui_tab_t* p_tab)
{
    ...
    while (NULL != p_next)
    {
        ...
        if (!is_tab_btn_klbui_tab(p_tab, p_cur) && !is_bind_wnd_klbui_tab(p_tab, p_cur))
        {
            ...
            p_tab->tab_btn[index] = klbui_tab_botton_create(p_wnd->p_gui, 120 * index, 0, 120, h);
...
static void on_klbui_tab_button_height(klb_wnd_t* p_wnd, klbui_tab_t* p_tab, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_attribute_int(&(p_tab->tab_btn_height), p_wnd, method, p_in, p_out);
}
...
static void klbui_tab_init_func_map(klb_wnd_t* p_wnd, klbui_tab_t* p_tab, klb_gui_t* p_gui)
{
    ...
    // 子控件-按钮-高度 button.height
    KLBUI_tab_bind("button.height", on_klbui_tab_button_height);
