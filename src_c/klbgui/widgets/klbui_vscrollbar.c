...
static klb_wnd_t* klbui_vscrollbar_create(klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    // step5. 默认使用公用全局CSS属性
    p_vsc->p_globalcss = (klbwnd_vscrollbar_css_t*)klb_gui_globalcss_get_ptr(p_gui, KLBUI_kvscrollbar);
...
{
    klbui_vscrollbar_init_globalcss(p_gui); // 初始化全局CSS, 需要注册时初始化, 否则全局属性无法生效
    return klb_gui_register(p_gui, KLBUI_kvscrollbar, klbui_vscrollbar_create);
}
