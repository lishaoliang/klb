...
static void klbui_list_destroy(klb_wnd_t* p_wnd)
{
    ...
    // 退出css
    klbwnd_list_css_quit(&p_list->css);
...
static void on_klbui_list_visibility(klb_wnd_t* p_wnd, klbui_list_t* p_list, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
...
static void klbui_list_init_func_map(klb_wnd_t* p_wnd, klbui_list_t* p_list, klb_gui_t* p_gui)
{
    ...
    // 显隐
    KLBUI_list_bind("visibility", on_klbui_list_visibility); // 显示/隐藏
...
klb_wnd_t* klbui_list_create(klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    // css
    klbwnd_list_css_init(&p_list->css, p_gui);
    klbwnd_list_set_css(p_wnd, &p_list->css);
