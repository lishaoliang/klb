...
static void klbui_view_on_paint_status(klb_wnd_t* p_wnd, klbui_view_t* p_view, klbuicssex_attributes_t* p_attr, klb_rect_t* p_rect)
{
    ...
    if (0 < sdslen(p_view->title))
    {
        ...
        // 移除内边距
        text_rect.x += p_view->padding.left;
        text_rect.y += p_view->padding.top;
        text_rect.w -= (p_view->padding.left + p_view->padding.right);
        text_rect.h -= (p_view->padding.top + p_view->padding.bottom);

        klb_wnd_draw_text2(p_wnd, &text_rect, p_view->title, sdslen(p_view->title), p_attr->text.color, p_attr->font.size);
    }
}
...
static int klbui_view_on_paint(klb_wnd_t* p_wnd)
{
    ...
    {
        klbui_view_on_paint_status(p_wnd, p_view, &p_view->disable, &paint_rect);
    }
    else if (KLB_WND_STATUS_FOCUS & p_wnd->state.status)
    {
        klbui_view_on_paint_status(p_wnd, p_view, &p_view->focus, &paint_rect);
    }
    else
    {
...
static void klbui_view_init_attribute(klb_wnd_t* p_wnd, klbui_view_t* p_view)
{
    ...
    klbuicssex_attributes_init(&p_view->focus, p_default);
    klbuicssex_attributes_init(&p_view->disable, p_default);
}
...
static void klbui_view_quit_attribute(klbui_view_t* p_view)
{
    ...
    klbuicssex_attributes_quit(&p_view->focus);
    klbuicssex_attributes_quit(&p_view->disable);
}
...
static void on_klbui_view_text_color_focus(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_text_color(&(p_view->focus.text), p_wnd, method, p_in, p_out);
}

static void on_klbui_view_text_color_disable(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_text_color(&(p_view->disable.text), p_wnd, method, p_in, p_out);
}
...
static void on_klbui_view_text_align_focus(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_text_align(&(p_view->focus.text), p_wnd, method, p_in, p_out);
}

static void on_klbui_view_text_align_disable(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_text_align(&(p_view->disable.text), p_wnd, method, p_in, p_out);
}
...
static void on_klbui_view_font_style_focus(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_font_style(&(p_view->focus.font), p_wnd, method, p_in, p_out);
}

static void on_klbui_view_font_style_disable(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_font_style(&(p_view->disable.font), p_wnd, method, p_in, p_out);
}
...
static void on_klbui_view_font_weight_focus(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_font_weight(&(p_view->focus.font), p_wnd, method, p_in, p_out);
}

static void on_klbui_view_font_weight_disable(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_font_weight(&(p_view->disable.font), p_wnd, method, p_in, p_out);
}
...
static void on_klbui_view_font_size_focus(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_font_size(&(p_view->focus.font), p_wnd, method, p_in, p_out);
}

static void on_klbui_view_font_size_disable(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_font_size(&(p_view->disable.font), p_wnd, method, p_in, p_out);
}
...
static void on_klbui_view_background_color_focus(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_background_color(&(p_view->focus.background), p_wnd, method, p_in, p_out);
}

static void on_klbui_view_background_color_disable(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_background_color(&(p_view->disable.background), p_wnd, method, p_in, p_out);
}
...
static void on_klbui_view_background_image_focus(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_background_image(&(p_view->focus.background), p_wnd, method, p_in, p_out);
}

static void on_klbui_view_background_image_disable(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_background_image(&(p_view->disable.background), p_wnd, method, p_in, p_out);
}
...
static void on_klbui_view_border_style_focus(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_border_style(&(p_view->focus.border), p_wnd, method, p_in, p_out);
}

static void on_klbui_view_border_style_disable(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_border_style(&(p_view->disable.border), p_wnd, method, p_in, p_out);
}
...
static void on_klbui_view_border_width_focus(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_border_width(&(p_view->focus.border), p_wnd, method, p_in, p_out);
}

static void on_klbui_view_border_width_disable(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_border_width(&(p_view->disable.border), p_wnd, method, p_in, p_out);
}
...
static void on_klbui_view_border_color_focus(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_border_color(&(p_view->focus.border), p_wnd, method, p_in, p_out);
}

static void on_klbui_view_border_color_disable(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_border_color(&(p_view->disable.border), p_wnd, method, p_in, p_out);
}
...
static void on_klbui_view_border_radius_focus(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_border_radius(&(p_view->focus.border), p_wnd, method, p_in, p_out);
}

static void on_klbui_view_border_radius_disable(klb_wnd_t* p_wnd, klbui_view_t* p_view, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_border_radius(&(p_view->disable.border), p_wnd, method, p_in, p_out);
}
...
static void klbui_view_init_func_map(klb_wnd_t* p_wnd, klbui_view_t* p_view, klb_gui_t* p_gui)
{
    ...
    KLBUI_view_bind("color:focus", on_klbui_view_text_color_focus);
    KLBUI_view_bind("color:disable", on_klbui_view_text_color_disable);

    // 文本对齐 text-align
    KLBUI_view_bind("text-align", on_klbui_view_text_align);
    KLBUI_view_bind("text-align:focus", on_klbui_view_text_align_focus);
    KLBUI_view_bind("text-align:disable", on_klbui_view_text_align_disable);

    // 斜体 font-style
    KLBUI_view_bind("font-style", on_klbui_view_font_style);
    KLBUI_view_bind("font-style:focus", on_klbui_view_font_style_focus);
    KLBUI_view_bind("font-style:disable", on_klbui_view_font_style_disable);

    // 字体粗细 font-weight
    KLBUI_view_bind("font-weight", on_klbui_view_font_weight);
    KLBUI_view_bind("font-weight:focus", on_klbui_view_font_weight_focus);
    KLBUI_view_bind("font-weight:disable", on_klbui_view_font_weight_disable);

    // 字体大小 font-size
    KLBUI_view_bind("font-size", on_klbui_view_font_size);
    KLBUI_view_bind("font-size:focus", on_klbui_view_font_size_focus);
    KLBUI_view_bind("font-size:disable", on_klbui_view_font_size_disable);

    // 背景色 background-color
    KLBUI_view_bind("background-color", on_klbui_view_background_color);
    KLBUI_view_bind("background-color:focus", on_klbui_view_background_color_focus);
    KLBUI_view_bind("background-color:disable", on_klbui_view_background_color_disable);

    // 背景图片 background-image
    KLBUI_view_bind("background-image", on_klbui_view_background_image);
    KLBUI_view_bind("background-image:focus", on_klbui_view_background_image_focus);
    KLBUI_view_bind("background-image:disable", on_klbui_view_background_image_disable);

    // 边框类型 border-style
    KLBUI_view_bind("border-style", on_klbui_view_border_style);
    KLBUI_view_bind("border-style:focus", on_klbui_view_border_style_focus);
    KLBUI_view_bind("border-style:disable", on_klbui_view_border_style_disable);

    // 边框的宽度 border-width
    KLBUI_view_bind("border-width", on_klbui_view_border_width);
    KLBUI_view_bind("border-width:focus", on_klbui_view_border_width_focus);
    KLBUI_view_bind("border-width:disable", on_klbui_view_border_width_disable);

    // 边框的颜色 border-color
    KLBUI_view_bind("border-color", on_klbui_view_border_color);
    KLBUI_view_bind("border-color:focus", on_klbui_view_border_color_focus);
    KLBUI_view_bind("border-color:disable", on_klbui_view_border_color_disable);

    // 圆角边框 border-radius
    KLBUI_view_bind("border-radius", on_klbui_view_border_radius);
    KLBUI_view_bind("border-radius:focus", on_klbui_view_border_radius_focus);
    KLBUI_view_bind("border-radius:disable", on_klbui_view_border_radius_disable);
