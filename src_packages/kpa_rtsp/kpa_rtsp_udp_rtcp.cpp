...
void kpa_rtsp_udp_rtcp_destroy(kpa_rtsp_udp_rtcp_t* p_udp)
{
...
int parse_kpa_rtsp_udp_rtcp(kpa_rtsp_udp_rtcp_t* p_tcp, klb_socket_t* p_socket)
{
...
static int on_remove_kpa_rtsp_udp_rtcp(void* p_lparam, void* p_wparam, int id)
{
    kpa_rtsp_udp_rtcp_t* p_tcp = (kpa_rtsp_udp_rtcp_t*)p_lparam;

    kpa_rtsp_udp_rtcp_destroy(p_tcp);

    return 0;
}

static int on_recv_kpa_rtsp_udp_rtcp(void* p_lparam, void* p_wparam, int id, int64_t now)
{
    kpa_rtsp_udp_rtcp_t* p_tcp = (kpa_rtsp_udp_rtcp_t*)p_lparam;
    ...
    while (true)
    {
        ...
        if (0 < r)
        {
            ...
            while (true)
            {
                if (0 != parse_kpa_rtsp_udp_rtcp(p_tcp, p_socket))
                {
                    break;
                }
            }
...
static int on_send_kpa_rtsp_udp_rtcp(void* p_lparam, void* p_wparam, int id, int64_t now)
{
    kpa_rtsp_udp_rtcp_t* p_tcp = (kpa_rtsp_udp_rtcp_t*)p_lparam;
    klb_socket_t* p_socket = p_tcp->p_socket;
...
{
    kpa_rtsp_udp_rtcp_t* p_udp = KLB_MALLOCZ(kpa_rtsp_udp_rtcp_t, 1, 0);
    ...
    o.cb_remove = on_remove_kpa_rtsp_udp_rtcp;
    o.cb_recv = on_recv_kpa_rtsp_udp_rtcp;
    o.cb_send = on_send_kpa_rtsp_udp_rtcp;
