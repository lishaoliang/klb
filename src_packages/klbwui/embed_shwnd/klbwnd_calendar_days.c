...
//////////////////////////////////////////////////////////////////////////
// 前置定义
static void klbwnd_calendar_days_deinit_attribute(klbwnd_calendar_days_t* p_days);
...
static void klbwnd_calendar_days_destroy(klb_wnd_t* p_wnd)
{
    ...
    klbwnd_calendar_days_deinit(p_wnd);
...
static void klbwnd_calendar_days_init_attribute(klbwnd_calendar_days_t* p_days)
{
    for (int i = 0; i < KLBWND_CAL_DAYS_column; i++)
    {
        p_days->weeks[i] = klb_sds_assign(NULL, s_klbwnd_calendar_days_weeks[i]);
    }
}
...
void klbwnd_calendar_days_deinit(klb_wnd_t* p_wnd)
{
    klbwnd_calendar_days_t* p_days = (klbwnd_calendar_days_t*)p_wnd->ctrl;

    klbwnd_calendar_days_deinit_attribute(p_days);
}
