...
static void klbwnd_decimal_menu_deinit_attribute(klbwnd_decimal_menu_t* p_menu);
static void klbwnd_decimal_menu_relayout(klb_wnd_t* p_wnd_menu, klbwnd_decimal_menu_t* p_menu);
static void klbwnd_decimal_menu_sync_title(klbwnd_decimal_menu_t* p_menu);
static void klbwnd_decimal_menu_popup_end(klb_wnd_t* p_wnd_menu, klbwnd_decimal_menu_t* p_menu, bool ok);
static void normalize_value_klbwnd_decimal_menu(klbwnd_decimal_menu_t* p_menu);


//////////////////////////////////////////////////////////////////////////
// 1080p 按键布局 (5 行 x 4 列)

static klbwnd_decimal_menu_layout_t s_klbwnd_decimal_menu_layout_1080p[KLBWND_DECMENU_row][KLBWND_DECMENU_column] = {
    {
        { { 2, 42, 122, 42 }, "7", KLBWND_DECMENU_KEY_normal },
        { { 126, 42, 122, 42 }, "8", KLBWND_DECMENU_KEY_normal },
        { { 250, 42, 122, 42 }, "9", KLBWND_DECMENU_KEY_normal },
        { { 0, 0, 0, 0 }, "", KLBWND_DECMENU_KEY_none }
    },
    {
        { { 2, 86, 122, 42 }, "4", KLBWND_DECMENU_KEY_normal },
        { { 126, 86, 122, 42 }, "5", KLBWND_DECMENU_KEY_normal },
        { { 250, 86, 122, 42 }, "6", KLBWND_DECMENU_KEY_normal },
        { { 0, 0, 0, 0 }, "", KLBWND_DECMENU_KEY_none }
    },
    {
        { { 2, 130, 122, 42 }, "1", KLBWND_DECMENU_KEY_normal },
        { { 126, 130, 122, 42 }, "2", KLBWND_DECMENU_KEY_normal },
        { { 250, 130, 122, 42 }, "3", KLBWND_DECMENU_KEY_normal },
        { { 0, 0, 0, 0 }, "", KLBWND_DECMENU_KEY_none }
    },
    {
        { { 2, 174, 122, 42 }, "0", KLBWND_DECMENU_KEY_normal },
        { { 126, 174, 60, 42 }, "<", KLBWND_DECMENU_KEY_moveleft },
        { { 188, 174, 60, 42 }, ">", KLBWND_DECMENU_KEY_moveright },
        { { 250, 174, 122, 42 }, "BS", KLBWND_DECMENU_KEY_backspace }
    },
    {
        { { 2, 218, 122, 42 }, "Clear", KLBWND_DECMENU_KEY_clear },
        { { 126, 218, 246, 42 }, "Enter", KLBWND_DECMENU_KEY_enter },
        { { 0, 0, 0, 0 }, "", KLBWND_DECMENU_KEY_none },
        { { 0, 0, 0, 0 }, "", KLBWND_DECMENU_KEY_none }
    }
};
...
static void klbwnd_decimal_menu_destroy(klb_wnd_t* p_wnd)
{
...
static void klbwnd_decimal_menu_on_paint_status(klb_wnd_t* p_wnd, klbwnd_decimal_menu_t* p_menu, klb_rect_t* p_rect)
{
    klbwnd_decimal_menu_css_t* p_css = &p_menu->css;
...
static int klbwnd_decimal_menu_on_paint(klb_wnd_t* p_wnd)
{
    klbwnd_decimal_menu_t* p_menu = (klbwnd_decimal_menu_t*)p_wnd->ctrl;
    ...
    klbwnd_decimal_menu_on_paint_status(p_wnd, p_menu, &paint_rect);

    return 0;
}

static int klbwnd_decimal_menu_on_control(klb_wnd_t* p_wnd, int msg, const klb_point_t* p_pt1, const klb_point_t* p_pt2, int lparam, int wparam)
{
    klbwnd_decimal_menu_t* p_menu = (klbwnd_decimal_menu_t*)p_wnd->ctrl;
    ...
    {
    case KLBUI_onpaint:
        return klbwnd_decimal_menu_on_paint(p_wnd);

    case KLBUI_outwindow:
        klbwnd_decimal_menu_popup_end(p_wnd, p_menu, true);
        break;
...
static int value_int_klbwnd_decimal_menu(klbwnd_decimal_menu_t* p_menu)
{
...
static void normalize_value_klbwnd_decimal_menu(klbwnd_decimal_menu_t* p_menu)
{
    ...
    snprintf(buf, sizeof(buf), "%d", value_int_klbwnd_decimal_menu(p_menu));
    buf[sizeof(buf) - 1] = '\0';
...
static void klbwnd_decimal_menu_popup_end(klb_wnd_t* p_wnd_menu, klbwnd_decimal_menu_t* p_menu, bool ok)
{
    if (NULL != p_menu->cb)
    {
        p_menu->cb(p_menu->ptr, p_wnd_menu, ok, value_int_klbwnd_decimal_menu(p_menu));
    }
...
static void klbwnd_decimal_menu_append(klb_wnd_t* p_wnd_menu, klbwnd_decimal_menu_t* p_menu, const char* p_str)
{
    ...
    if (val_len < p_menu->max_len)
    {
        ...
        normalize_value_klbwnd_decimal_menu(p_menu);
        klbwnd_decimal_menu_sync_title(p_menu);
        klb_wnd_update(p_wnd_menu);
    }
    else if (0 == value_int_klbwnd_decimal_menu(p_menu))
    {
...
static void klbwnd_decimal_menu_dec(klb_wnd_t* p_wnd_menu, klbwnd_decimal_menu_t* p_menu)
{
    int char_num = 0;
    int del_pos = 0;
...
static void klbwnd_decimal_menu_clear(klb_wnd_t* p_wnd_menu, klbwnd_decimal_menu_t* p_menu)
{
...
static void klbwnd_decimal_menu_cursor_moveleft(klb_wnd_t* p_wnd_menu, klbwnd_decimal_menu_t* p_menu)
{
...
static void klbwnd_decimal_menu_cursor_moveright(klb_wnd_t* p_wnd_menu, klbwnd_decimal_menu_t* p_menu)
{
...
static int on_click_btn_klbwnd_decimal_menu(klb_wnd_t* p_wnd_btn, klb_wnd_t* p_wnd_menu, klbwnd_decimal_menu_t* p_menu, int type)
{
    if (KLBWND_DECMENU_KEY_normal == type)
    {
        const sds value = klbwnd_button_get_value(p_wnd_btn);
        if (NULL != value && 0 < sdslen(value))
        {
            klbwnd_decimal_menu_append(p_wnd_menu, p_menu, value);
        }
    }
    else if (KLBWND_DECMENU_KEY_backspace == type)
    {
        klbwnd_decimal_menu_dec(p_wnd_menu, p_menu);
    }
    else if (KLBWND_DECMENU_KEY_moveleft == type)
    {
        klbwnd_decimal_menu_cursor_moveleft(p_wnd_menu, p_menu);
    }
    else if (KLBWND_DECMENU_KEY_moveright == type)
    {
        klbwnd_decimal_menu_cursor_moveright(p_wnd_menu, p_menu);
    }
    else if (KLBWND_DECMENU_KEY_clear == type)
    {
        klbwnd_decimal_menu_clear(p_wnd_menu, p_menu);
    }
...
static int on_command_btn_klbwnd_decimal_menu(klb_wnd_t* p_wnd_btn, int msg, const klb_point_t* p_pt1, const klb_point_t* p_pt2, int lparam, int wparam)
{
    ...
    if (KLBUI_mousedown == msg)
    {
        ...
        {
            if (KLBWND_DECMENU_KEY_enter != type)
            {
                return on_click_btn_klbwnd_decimal_menu(p_wnd_btn, p_wnd_menu, p_menu, type);
            }
        }
    }
    else if (KLBUI_click == msg || KLBUI_dblclick == msg)
    {
        if (KLBWND_DECMENU_KEY_enter == type)
        {
            klbwnd_decimal_menu_popup_end(p_wnd_menu, p_menu, true);
        }
    }
...
static void klbwnd_decimal_menu_relayout(klb_wnd_t* p_wnd_menu, klbwnd_decimal_menu_t* p_menu)
{
    for (int m = 0; m < KLBWND_DECMENU_row; ++m)
    {
        for (int n = 0; n < KLBWND_DECMENU_column; ++n)
        {
...
static int klbwnd_decimal_menu_init_subwnd(klb_wnd_t* p_wnd_menu, klbwnd_decimal_menu_t* p_menu, klb_gui_t* p_gui)
{
    memcpy(p_menu->layout, s_klbwnd_decimal_menu_layout_1080p, sizeof(s_klbwnd_decimal_menu_layout_1080p));
    ...
    for (int m = 0; m < KLBWND_DECMENU_row; ++m)
    {
        for (int n = 0; n < KLBWND_DECMENU_column; ++n)
        {
            ...
            klb_wnd_bind_command(p_wnd_btn, on_command_btn_klbwnd_decimal_menu, p_wnd_menu);
            klb_wnd_show(p_wnd_btn, false);
...
static void klbwnd_decimal_menu_fg_deinit(klbwnd_decimal_menu_fg_t* p_fg)
{
...
void klbwnd_decimal_menu_css_deinit(klbwnd_decimal_menu_t* p_menu)
{
    ...
    klbwnd_decimal_menu_fg_deinit(&p_menu->css.btn_attr.normal_foreground);
    klbwnd_decimal_menu_fg_deinit(&p_menu->css.btn_attr.focus_foreground);
    klbwnd_decimal_menu_fg_deinit(&p_menu->css.btn_attr.disable_foreground);
}
...
{
    klbwnd_decimal_menu_t* p_self = (klbwnd_decimal_menu_t*)p_wnd->ctrl;
...
int klbwnd_decimal_menu_get_value(klb_wnd_t* p_wnd)
{
    ...
    return value_int_klbwnd_decimal_menu(p_menu);
}
...
int klbwnd_decimal_menu_bind(klb_wnd_t* p_wnd, klbshw_decimal_menu_cb cb, void* ptr)
{
    klbwnd_decimal_menu_t* p_menu = (klbwnd_decimal_menu_t*)p_wnd->ctrl;
...
int klbwnd_decimal_menu_layout(klb_wnd_t* p_wnd)
{
    ...
    klbwnd_decimal_menu_relayout(p_wnd, p_menu);
...
void klbwnd_decimal_menu_init(klb_wnd_t* p_wnd, klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    p_wnd->vtable.on_control = klbwnd_decimal_menu_on_control;
    ...
    klbwnd_decimal_menu_init_attribute(p_wnd, p_menu);
    klbwnd_decimal_menu_init_subwnd(p_wnd, p_menu, p_gui);
}
...
void klbwnd_decimal_menu_deinit(klb_wnd_t* p_wnd)
{
    ...
    klbwnd_decimal_menu_deinit_attribute(p_menu);
}

klb_wnd_t* klbwnd_decimal_menu_create(klb_gui_t* p_gui, int x, int y, int w, int h)
{
    klb_wnd_t* p_wnd = KLB_MALLOCZ(klb_wnd_t, 1, sizeof(klbwnd_decimal_menu_t));

    klbwnd_decimal_menu_init(p_wnd, p_gui, x, y, w, h);
    p_wnd->vtable.destroy = klbwnd_decimal_menu_destroy;
