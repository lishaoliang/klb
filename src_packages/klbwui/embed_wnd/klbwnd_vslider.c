...
static void klbwnd_vslider_on_paint_status(klb_wnd_t* p_wnd, klbwnd_vslider_t* p_vslider, klbwnd_vslider_css_t* p_css, klbwnd_vslider_attributes_t* p_attr, klb_rect_t* p_rect)
{
...
static void klbwnd_vslider_on_click(klb_wnd_t* p_wnd, klbwnd_vslider_t* p_vslider, const klb_point_t* p_pt1)
{
    ...
    // 鼠标点击坐标存在精度问题, 较难以命中两端; 用 padding.top / padding.bottom 消除操作精度
    ...
    if (value <= p_vslider->min)
    {
        value = p_vslider->min;
    }
    else if (value >= p_vslider->max)
    {
        value = p_vslider->max;
    }
...
static void klbwnd_vslider_init_attribute(klbwnd_vslider_t* p_vslider)
{
    p_vslider->min = 0;
    p_vslider->max = 100;
    p_vslider->step = 1;
...
static void klbwnd_vslider_attributes_init(klbwnd_vslider_attributes_t* p_attr, const klbuicssex_attributes_t* p_src)
{
    ...
    p_attr->thumb_image = NULL;
}

static void klbwnd_vslider_attributes_deinit(klbwnd_vslider_attributes_t* p_attr)
{
    KLB_FREE_BY(p_attr->thumb_image, sdsfree);
}

static void klbwnd_vslider_attributes_copy(klbwnd_vslider_attributes_t* p_dst, const klbwnd_vslider_attributes_t* p_src)
{
    ...
    p_dst->thumb_image = klb_sds_assign(p_dst->thumb_image, p_src->thumb_image);
}
...
void klbwnd_vslider_css_init(klbwnd_vslider_css_t* p_css, klb_gui_t* p_gui)
{
    ...
    klbwnd_vslider_attributes_init(&p_css->normal, &p_default->normal);
    klbwnd_vslider_attributes_init(&p_css->focus, &p_default->focus);
    klbwnd_vslider_attributes_init(&p_css->disable, &p_default->disable);

    p_css->foreground_width = 6;
    p_css->thumb_width = 12;
    p_css->thumb_height = 6;
...
{
    klbwnd_vslider_attributes_deinit(&p_css->normal);
    klbwnd_vslider_attributes_deinit(&p_css->focus);
    klbwnd_vslider_attributes_deinit(&p_css->disable);
}

void klbwnd_vslider_css_copy(klbwnd_vslider_css_t* p_dst, klbwnd_vslider_css_t* p_src)
{
    p_dst->foreground_width = p_src->foreground_width;
    p_dst->thumb_width = p_src->thumb_width;
    p_dst->thumb_height = p_src->thumb_height;

    p_dst->margin = p_src->margin;
    p_dst->padding = p_src->padding;

    klbwnd_vslider_attributes_copy(&p_dst->normal, &p_src->normal);
    klbwnd_vslider_attributes_copy(&p_dst->focus, &p_src->focus);
    klbwnd_vslider_attributes_copy(&p_dst->disable, &p_src->disable);
}
...
void klbwnd_vslider_init(klb_wnd_t* p_wnd, klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    klbwnd_vslider_init_attribute(p_vslider);
}
...
void klbwnd_vslider_deinit(klb_wnd_t* p_wnd)
{
    ...
    klbwnd_vslider_deinit_attribute(p_vslider);
}
