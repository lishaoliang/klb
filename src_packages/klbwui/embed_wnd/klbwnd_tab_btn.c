...
#include "klbwui/embed_wnd/klbwnd_tab_btn.h"
...
//////////////////////////////////////////////////////////////////////////
// 前置定义
static void klbwnd_tab_btn_deinit_attribute(klbwnd_tab_btn_t* p_btn);
...
static void klbwnd_tab_btn_destroy(klb_wnd_t* p_wnd)
{
    ...
    klbwnd_tab_btn_deinit(p_wnd);

    KLB_FREE(p_wnd);
}

static void klbwnd_tab_btn_on_paint_status(klb_wnd_t* p_wnd, klbwnd_tab_btn_t* p_btn, klbwnd_tab_btn_css_t* p_css, klbwnd_tab_btn_attributes_t* p_attr, klb_rect_t* p_rect)
{
...
void klbwnd_tab_btn_update_title(klb_wnd_t* p_wnd)
{
    ...
    if (NULL != p_out)
    {
        ...
        if (NULL != p_tile)
        {
            p_btn->title = klb_sds_assign(p_btn->title, p_tile);
        }
...
int klbwnd_tab_btn_title_size(klb_wnd_t* p_wnd, int* p_out_w, int* p_out_h)
{
    ...
    if (NULL != p_btn->p_css && NULL != p_btn->title && 0 < sdslen(p_btn->title))
    {
        // 按常规状态测算标题长度
...
static void klbwnd_tab_btn_attributes_init(klbwnd_tab_btn_attributes_t* p_attr, const klbuicssex_attributes_t* p_src)
{
...
static void klbwnd_tab_btn_attributes_deinit(klbwnd_tab_btn_attributes_t* p_attr)
{
    KLB_FREE_BY(p_attr->background.image, sdsfree);
}

static void klbwnd_tab_btn_attributes_copy(klbwnd_tab_btn_attributes_t* p_dst, const klbwnd_tab_btn_attributes_t* p_src)
{
...
void klbwnd_tab_btn_css_init(klbwnd_tab_btn_css_t* p_css, klb_gui_t* p_gui)
{
    ...
    klbwnd_tab_btn_attributes_init(&p_css->normal, &p_default->normal);
    klbwnd_tab_btn_attributes_init(&p_css->focus, &p_default->focus);
    klbwnd_tab_btn_attributes_init(&p_css->disable, &p_default->disable);
...
{
    klbwnd_tab_btn_attributes_deinit(&p_css->normal);
    klbwnd_tab_btn_attributes_deinit(&p_css->focus);
    klbwnd_tab_btn_attributes_deinit(&p_css->disable);
}
...
void klbwnd_tab_btn_css_copy(klbwnd_tab_btn_css_t* p_dst, klbwnd_tab_btn_css_t* p_src)
{
    ...
    klbwnd_tab_btn_attributes_copy(&p_dst->normal, &p_src->normal);
    klbwnd_tab_btn_attributes_copy(&p_dst->focus, &p_src->focus);
    klbwnd_tab_btn_attributes_copy(&p_dst->disable, &p_src->disable);
}
...
void klbwnd_tab_btn_deinit(klb_wnd_t* p_wnd)
{
    ...
    klbwnd_tab_btn_deinit_attribute(p_btn);
}
