...
static void klbwnd_line_destroy(klb_wnd_t* p_wnd)
{
...
static void klbwnd_line_draw_background_and_border(klb_wnd_t* p_wnd, klbwnd_line_attributes_t* p_attr, klb_rect_t* p_rect)
{
...
static int klbwnd_line_on_paint(klb_wnd_t* p_wnd)
{
    klbwnd_line_t* p_line = (klbwnd_line_t*)p_wnd->ctrl;
    ...
    klbwnd_line_css_t* p_css = p_line->p_css;
    ...
    klbwnd_line_draw_background_and_border(p_wnd, &p_css->normal, &paint_rect);
...
static int klbwnd_line_on_control(klb_wnd_t* p_wnd, int msg, const klb_point_t* p_pt1, const klb_point_t* p_pt2, int lparam, int wparam)
{
    ...
    {
    case KLBUI_onpaint:
        return klbwnd_line_on_paint(p_wnd);
    default:
        break;
    }
...
static void klbwnd_line_attributes_init(klbwnd_line_attributes_t* p_attr, const klbuicssex_attributes_t* p_src)
{
...
static void klbwnd_line_attributes_deinit(klbwnd_line_attributes_t* p_attr)
{
    KLB_FREE_BY(p_attr->background.image, sdsfree);
}

static void klbwnd_line_attributes_copy(klbwnd_line_attributes_t* p_dst, const klbwnd_line_attributes_t* p_src)
{
...
void klbwnd_line_css_init(klbwnd_line_css_t* p_css, klb_gui_t* p_gui)
{
    ...
    klbwnd_line_attributes_init(&p_css->normal, &p_default->normal);
}

void klbwnd_line_css_deinit(klbwnd_line_css_t* p_css)
{
    klbwnd_line_attributes_deinit(&p_css->normal);
}
...
void klbwnd_line_css_copy(klbwnd_line_css_t* p_dst, klbwnd_line_css_t* p_src)
{
    ...
    klbwnd_line_attributes_copy(&p_dst->normal, &p_src->normal);
}
...
void klbwnd_line_init(klb_wnd_t* p_wnd, klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    p_wnd->vtable.on_control = klbwnd_line_on_control;
...
{
    klb_wnd_t* p_wnd = KLB_MALLOCZ(klb_wnd_t, 1, sizeof(klbwnd_line_t));

    klbwnd_line_init(p_wnd, p_gui, x, y, w, h);

    p_wnd->vtable.destroy = klbwnd_line_destroy;
