...
//////////////////////////////////////////////////////////////////////////
// 前置定义
static void klbwnd_menu_deinit_attribute(klbwnd_menu_t* p_menu);
...
static void klbwnd_menu_on_paint_status(klb_wnd_t* p_wnd, klbwnd_menu_t* p_menu, klbwnd_menu_css_t* p_css, klbwnd_menu_attributes_t* p_attr, klb_rect_t* p_rect)
{
    // 纯色背景
...
static void popup_2rd_klbwnd_menu(klb_wnd_t* p_wnd, klbwnd_menu_t* p_menu, const klb_rect_t* p_rect, const char* p_key)
{
    ...
    p_menu->key_2rd = klb_sds_assign(p_menu->key_2rd, p_key);
}
...
static int on_command_2rd_klbwnd_menu(klb_wnd_t* p_wnd, int msg, const klb_point_t* p_pt1, const klb_point_t* p_pt2, int lparam, int wparam)
{
    ...
    {
        // 保存值
        p_menu->value = klb_sds_assign(p_menu->value, klbwnd_menu_2rd_get_value(p_wnd));
...
static klb_map_t* find_map_by_value_klbwnd_menu(klb_map_t* p_map, const char* p_str)
{
    int count = klb_map_array_size(p_map);
    for (int i = 0; i < count; i++)
    {
        klb_map_t* p_item = klb_map_idx_to_map(p_map, i);
        ...
        const char* p_value = klb_map_to_string(p_item, "value");
        if (NULL != p_value && NULL != p_str)
        {
            if (0 == strcmp(p_value, p_str))
            {
                return p_item;
            }
        }
...
static int update_map_item_klbwnd_menu(klb_map_t* p_dst, klb_map_t* p_array)
{
    int count = klb_map_array_size(p_array);
    for (int i = 0; i < count; i++)
    {
        klb_map_t* p_item = klb_map_idx_to_map(p_array, i);
        if (NULL == p_item)
        {
            continue;   // 不是 table
        }
        ...
        const char* p_value = klb_map_to_string(p_item, "value");
        if (NULL == p_value || strlen(p_value) <= 0)
        {
            continue;   // 无value值, 或者空
        }

        klb_map_t* p_src = find_map_by_value_klbwnd_menu(p_dst, p_value);
        if (NULL == p_src)
        {
            continue;   // 未找到源
        }
        ...
        // 标题 "title"
        if (KLB_ADT_string == klb_map_type(p_item, "title"))
        {
            klb_map_set_string(p_src, "title", klb_map_to_string(p_item, "title"));
        }

        // 显影 "visibility"
        if (KLB_ADT_bool == klb_map_type(p_item, "visibility"))
        {
...
void klbwnd_menu_item_update(klb_wnd_t* p_wnd, klb_map_t* p_array)
{
    ...
    update_map_item_klbwnd_menu(&p_menu->data, p_array);
}
...
void klbwnd_menu_item_2rd_update(klb_wnd_t* p_wnd, klb_map_t* p_2rd)
{
    ...
    while (NULL != p_iter)
    {
        ...
        klb_map_t* p_dst = klb_map_to_map(&p_menu->data_2rd, p_key);

        if (NULL != p_dst)
        {
            klb_map_t* p_array = klb_adt_to_map(klb_map_data(p_iter));
            update_map_item_klbwnd_menu(p_dst, p_array);
        }
...
void klbwnd_menu_css_init(klbwnd_menu_css_t* p_css, klb_gui_t* p_gui)
{
    ...
    klbwnd_menu_attributes_init(&p_css->normal, &p_default->normal);
...
{
    klbwnd_menu_attributes_deinit(&p_css->normal);

    klbwnd_menu_item_css_deinit(&p_css->css_item);
}
...
void klbwnd_menu_css_copy(klbwnd_menu_css_t* p_dst, klbwnd_menu_css_t* p_src)
{
    ...
    klbwnd_menu_attributes_copy(&p_dst->normal, &p_src->normal);
...
void klbwnd_menu_deinit(klb_wnd_t* p_wnd)
{
    ...
    // 退出属性
    klbwnd_menu_deinit_attribute(p_menu);
}
