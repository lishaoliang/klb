...
//////////////////////////////////////////////////////////////////////////
// 前置定义
static void klbwnd_listex_row_deinit_attribute(klbwnd_listex_row_t* p_row);
...
static void klbwnd_listex_row_destroy(klb_wnd_t* p_wnd)
{
    ...
    klbwnd_listex_row_deinit(p_wnd);
...
static void klbwnd_listex_row_on_paint_status(klb_wnd_t* p_wnd, klbwnd_listex_row_t* p_row, klbwnd_listex_row_css_t* p_css, klbwnd_listex_row_attributes_t* p_attr, klb_rect_t* p_rect, uint32_t background_color2)
{
    if (p_row->b_select)
    {
        // 选中状态
...
static void klbwnd_listex_row_init_attribute(klb_wnd_t* p_wnd, klbwnd_listex_row_t* p_row)
{
    ...
    p_row->b_check = false;
    p_row->title = sdsempty(); /* scratch only: column draw buffer, not business default sds */
}
...
static void klbwnd_listex_row_attributes_init(klbwnd_listex_row_attributes_t* p_attr, const klbuicssex_attributes_t* p_src)
{
...
static void klbwnd_listex_row_attributes_deinit(klbwnd_listex_row_attributes_t* p_attr)
{
    KLB_FREE_BY(p_attr->background.image, sdsfree);
}

static void klbwnd_listex_row_attributes_copy(klbwnd_listex_row_attributes_t* p_dst, const klbwnd_listex_row_attributes_t* p_src)
{
...
void klbwnd_listex_row_css_init(klbwnd_listex_row_css_t* p_css, klb_gui_t* p_gui)
{
    ...
    klbwnd_listex_row_attributes_init(&p_css->normal, &p_default->normal);
    klbwnd_listex_row_attributes_init(&p_css->focus, &p_default->focus);
    klbwnd_listex_row_attributes_init(&p_css->disable, &p_default->disable);
...
void klbwnd_listex_row_css_deinit(klbwnd_listex_row_css_t* p_css)
{
    klbwnd_listex_row_attributes_deinit(&p_css->normal);
    klbwnd_listex_row_attributes_deinit(&p_css->focus);
    klbwnd_listex_row_attributes_deinit(&p_css->disable);
}
...
void klbwnd_listex_row_css_copy(klbwnd_listex_row_css_t* p_dst, klbwnd_listex_row_css_t* p_src)
{
    ...
    klbwnd_listex_row_attributes_copy(&p_dst->normal, &p_src->normal);
    klbwnd_listex_row_attributes_copy(&p_dst->focus, &p_src->focus);
    klbwnd_listex_row_attributes_copy(&p_dst->disable, &p_src->disable);
...
void klbwnd_listex_row_deinit(klb_wnd_t* p_wnd)
{
    klbwnd_listex_row_t* p_row = (klbwnd_listex_row_t*)p_wnd->ctrl;

    klbwnd_listex_row_deinit_attribute(p_row);
}
