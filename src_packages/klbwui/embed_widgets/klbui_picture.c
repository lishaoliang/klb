...
/// @brief  图片框 (Lua type: kpicture)
typedef struct klbui_picture_t_
{
    klbwnd_picture_t            picture;        ///< 必须首位, 保持内存一致

    klbwnd_picture_css_t*       p_globalcss;    ///< 全局公共的 CSS
    klbwnd_picture_css_t*       p_mycss;        ///< 私有的 CSS
...
static void klbui_picture_destroy(klb_wnd_t* p_wnd)
{
    ...
    {
        klbwnd_picture_css_deinit(p_pic->p_mycss);
...
static void globalcss_klbui_picture_background_image_mode(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_picture_css_t* p_css = (klbwnd_picture_css_t*)ptr;
...
static void globalcss_klbui_picture_background_image_color_key(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_picture_css_t* p_css = (klbwnd_picture_css_t*)ptr;
...
static void destroy_globalcss_klbwnd_picture(void* ptr)
{
    ...
    klbwnd_picture_css_deinit(p_css);
    KLB_FREE(p_css);
}
...
static void on_klbui_picture_background_image_mode(klb_wnd_t* p_wnd, klbui_picture_t* p_pic, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_picture_css_t* p_css = check_css_klbui_picture(p_wnd, method);
...
static void on_klbui_picture_background_image_color_key(klb_wnd_t* p_wnd, klbui_picture_t* p_pic, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_picture_css_t* p_css = check_css_klbui_picture(p_wnd, method);
...
{
    // step1. 获取CSS 支持的方法
    klb_map_t* ptr = klb_gui_css_map(p_gui, KLBWUI_kpicture);
    ...
    ptr = klb_gui_new_css_map(p_gui, KLBWUI_kpicture);
    p_pic->p_func_map = ptr;
...
static klb_wnd_t* klbui_picture_create(klb_gui_t* p_gui, int x, int y, int w, int h)
{
    ...
    // step5. 默认使用公用全局CSS属性
    p_pic->p_globalcss = (klbwnd_picture_css_t*)klb_gui_globalcss_get_ptr(p_gui, KLBWUI_kpicture);
...
int klbui_register_kpicture(klb_gui_t* p_gui)
{
    ...
    return klb_gui_register(p_gui, KLBWUI_kpicture, klbui_picture_create);
}
