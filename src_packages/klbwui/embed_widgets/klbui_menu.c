// Doc Encode : UTF-8 BOM, Unix(LF)
#include "klbwui/core/klbwui_widgets.h"
#include "klbwui/core/klbwui_css.h"
#include "klbwui/embed_wnd/klbwnd_menu.h"
#include "klbmem/klb_mem.h"


/// @struct klbui_menu_t
/// @brief  菜单
typedef struct klbui_menu_t_
{
    klbwnd_menu_t       menu;           ///< 必须首位, 保持内存一致

    klbwnd_menu_css_t*  p_globalcss;    ///< 全局公共的CSS
    klbwnd_menu_css_t*  p_mycss;        ///< 私有的CSS

    klb_map_t*          p_func_map;     ///< CSS属性函数表
}klbui_menu_t;


//////////////////////////////////////////////////////////////////////////
// 前置定义
typedef void(*klbui_menu_cb)(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out);


//////////////////////////////////////////////////////////////////////////
// 继承/重写方法

static void klbui_menu_destroy(klb_wnd_t* p_wnd)
{
    klbui_menu_t* p_menu = (klbui_menu_t*)p_wnd->ctrl;

    // 退出基础部分
    klbwnd_menu_deinit(p_wnd);

    // 退出css
    if (NULL != p_menu->p_mycss)
    {
        klbwnd_menu_css_deinit(p_menu->p_mycss);
        KLB_FREE(p_menu->p_mycss);
    }

    KLB_FREE(p_wnd);
}

static int klbui_menu_on_set(klb_wnd_t* p_wnd, const klb_map_t* p_map)
{
    klbui_menu_t* p_menu = (klbui_menu_t*)p_wnd->ctrl;

    const char* p_key = klb_map_idx_to_string(p_map, 0);
    klbui_menu_cb func = (klbui_menu_cb)klb_map_to_ptr(p_menu->p_func_map, p_key, NULL);

    if (NULL != func)
    {
        func(p_wnd, p_menu, KLBUI_CSSEX_set, p_map, NULL);
    }

    return 0;
}

static klb_map_t* klbui_menu_on_get(klb_wnd_t* p_wnd, const klb_map_t* p_map)
{
    klbui_menu_t* p_menu = (klbui_menu_t*)p_wnd->ctrl;
    klb_map_t* p_out = klb_map_create();

    const char* p_key = klb_map_idx_to_string(p_map, 0);
    klbui_menu_cb func = (klbui_menu_cb)klb_map_to_ptr(p_menu->p_func_map, p_key, NULL);

    if (NULL != func)
    {
        func(p_wnd, p_menu, KLBUI_CSSEX_get, p_map, p_out);
    }

    return p_out;
}

//////////////////////////////////////////////////////////////////////////
// my css

// 新建私有CSS
static void new_my_css_klbui_menu(klb_wnd_t* p_wnd)
{
    klbui_menu_t* p_btn = (klbui_menu_t*)p_wnd->ctrl;

    if (NULL == p_btn->p_mycss)
    {
        p_btn->p_mycss = KLB_MALLOCZ(klbwnd_menu_css_t, 1, 0);

        // 从新建时, 复制公共参数; 后面分开
        klbwnd_menu_css_copy(p_btn->p_mycss, p_btn->p_globalcss);
        klbwnd_menu_set_css(p_wnd, p_btn->p_mycss);
    }
}

// 获取当前 CSS
static klbwnd_menu_css_t* get_css_klbui_menu(klb_wnd_t* p_wnd)
{
    klbui_menu_t* p_btn = (klbui_menu_t*)p_wnd->ctrl;
    return (NULL != p_btn->p_mycss) ? p_btn->p_mycss : p_btn->p_globalcss;
}

// 检查方法, 并获取当前的CSS
static klbwnd_menu_css_t* check_css_klbui_menu(klb_wnd_t* p_wnd, int method)
{
    if (KLBUI_CSSEX_set == method)
    {
        // 只要是 set-css 相关方法, 立即转换为使用私有CSS
        new_my_css_klbui_menu(p_wnd);
    }

    return get_css_klbui_menu(p_wnd);
}

//////////////////////////////////////////////////////////////////////////
// global CSS 方法 - 公共CSS属性

static void globalcss_klbui_menu_margin(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_margin(&(p_css->margin), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_margin_top(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_margin_top(&(p_css->margin), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_margin_right(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_margin_right(&(p_css->margin), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_margin_bottom(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_margin_bottom(&(p_css->margin), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_margin_left(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_margin_left(&(p_css->margin), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_padding(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_padding(&(p_css->padding), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_padding_top(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_padding_top(&(p_css->padding), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_padding_right(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_padding_right(&(p_css->padding), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_padding_bottom(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_padding_bottom(&(p_css->padding), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_padding_left(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_padding_left(&(p_css->padding), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_text_color(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_text_color(&(p_css->normal.text), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_text_align(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_text_align(&(p_css->normal.text), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_font_size(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_font_size(&(p_css->normal.font), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_background_color(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_background_color(&(p_css->normal.background), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_background_image(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_background_image(&(p_css->normal.background), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_border_width(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_border_width(&(p_css->normal.border), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_border_color(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_border_color(&(p_css->normal.border), NULL, method, p_in, p_out);
}

////////////////////////////////////////
// 子项

static void globalcss_klbui_menu_item_text_color(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_text_color(&(p_css->css_item.normal.text), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_item_text_color_focus(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_text_color(&(p_css->css_item.focus.text), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_item_text_color_disable(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_text_color(&(p_css->css_item.disable.text), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_item_text_align(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_text_align(&(p_css->css_item.normal.text), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_item_text_align_focus(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_text_align(&(p_css->css_item.focus.text), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_item_text_align_disable(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_text_align(&(p_css->css_item.disable.text), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_item_font_size(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_font_size(&(p_css->css_item.normal.font), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_item_font_size_focus(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_font_size(&(p_css->css_item.focus.font), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_item_font_size_disable(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_font_size(&(p_css->css_item.disable.font), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_item_background_color(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_background_color(&(p_css->css_item.normal.background), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_item_background_color_focus(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_background_color(&(p_css->css_item.focus.background), NULL, method, p_in, p_out);
}

static void globalcss_klbui_menu_item_background_color_disable(void* ptr, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbuicssex_background_color(&(p_css->css_item.disable.background), NULL, method, p_in, p_out);
}

//////////////////////////////////////

/// @brief 销毁 全局公共CSS属性
static void destroy_globalcss_klbwnd_menu(void* ptr)
{
    klbwnd_menu_css_t* p_css = (klbwnd_menu_css_t*)ptr;
    klbwnd_menu_css_deinit(p_css);
    KLB_FREE(p_css);
}

#define KLBUI_GLOBAL_menu_bind(KEY_, FUNC_) { klb_map_set_ptr(ptr, (KEY_), (void*)(FUNC_), NULL); }

static void klbui_menu_init_globalcss(klb_gui_t* p_gui)
{
    klb_map_t* ptr = klb_gui_globalcss_map(p_gui, KLBWUI_kmenu);
    if (NULL != ptr)
    {
        return;
    }

    // 未找到, 则新添加 解析map, 及处理函数
    ptr = klb_gui_new_globalcss_map(p_gui, KLBWUI_kmenu);

    // 添加全局 CSS 
    klbwnd_menu_css_t* p_css = KLB_MALLOCZ(klbwnd_menu_css_t, 1, 0);
    klbwnd_menu_css_init(p_css, p_gui);
    klb_gui_globalcss_set_ptr(p_gui, KLBWUI_kmenu, p_css, destroy_globalcss_klbwnd_menu);

    //////////////////////////////////////////////
    // 若第一次, 则添加全局属性解析方法
    // 仿 CSS 方法 - 全局CSS属性

    // 外边距 margin
    KLBUI_GLOBAL_menu_bind("margin", globalcss_klbui_menu_margin);
    KLBUI_GLOBAL_menu_bind("margin-top", globalcss_klbui_menu_margin_top);
    KLBUI_GLOBAL_menu_bind("margin-right", globalcss_klbui_menu_margin_right);
    KLBUI_GLOBAL_menu_bind("margin-bottom", globalcss_klbui_menu_margin_bottom);
    KLBUI_GLOBAL_menu_bind("margin-left", globalcss_klbui_menu_margin_left);

    // 内边距 padding
    KLBUI_GLOBAL_menu_bind("padding", globalcss_klbui_menu_padding);
    KLBUI_GLOBAL_menu_bind("padding-top", globalcss_klbui_menu_padding_top);
    KLBUI_GLOBAL_menu_bind("padding-right", globalcss_klbui_menu_padding_right);
    KLBUI_GLOBAL_menu_bind("padding-bottom", globalcss_klbui_menu_padding_bottom);
    KLBUI_GLOBAL_menu_bind("padding-left", globalcss_klbui_menu_padding_left);

    // 文本颜色 color
    KLBUI_GLOBAL_menu_bind("color", globalcss_klbui_menu_text_color);

    // 文本对齐 text-align
    KLBUI_GLOBAL_menu_bind("text-align", globalcss_klbui_menu_text_align);

    // 字体大小 font-size
    KLBUI_GLOBAL_menu_bind("font-size", globalcss_klbui_menu_font_size);

    // 背景色 background-color
    KLBUI_GLOBAL_menu_bind("background-color", globalcss_klbui_menu_background_color);

    // 背景图片 background-image
    KLBUI_GLOBAL_menu_bind("background-image", globalcss_klbui_menu_background_image);

    // 边框的宽度 border-width
    KLBUI_GLOBAL_menu_bind("border-width", globalcss_klbui_menu_border_width);

    // 边框的颜色 border-color
    KLBUI_GLOBAL_menu_bind("border-color", globalcss_klbui_menu_border_color);

    //////////////////////////////////////////////
    // 子项

    // 子项.文本颜色 color
    KLBUI_GLOBAL_menu_bind("item.color", globalcss_klbui_menu_item_text_color);
    KLBUI_GLOBAL_menu_bind("item.color:focus", globalcss_klbui_menu_item_text_color_focus);
    KLBUI_GLOBAL_menu_bind("item.color:disabled", globalcss_klbui_menu_item_text_color_disable);

    // 子项.文本对齐 text-align
    KLBUI_GLOBAL_menu_bind("item.text-align", globalcss_klbui_menu_item_text_align);
    KLBUI_GLOBAL_menu_bind("item.text-align:focus", globalcss_klbui_menu_item_text_align_focus);
    KLBUI_GLOBAL_menu_bind("item.text-align:disabled", globalcss_klbui_menu_item_text_align_disable);

    // 子项.字体大小 font-size
    KLBUI_GLOBAL_menu_bind("item.font-size", globalcss_klbui_menu_item_font_size);
    KLBUI_GLOBAL_menu_bind("item.font-size:focus", globalcss_klbui_menu_item_font_size_focus);
    KLBUI_GLOBAL_menu_bind("item.font-size:disabled", globalcss_klbui_menu_item_font_size_disable);

    // 子项.背景色 background-color
    KLBUI_GLOBAL_menu_bind("item.background-color", globalcss_klbui_menu_item_background_color);
    KLBUI_GLOBAL_menu_bind("item.background-color:focus", globalcss_klbui_menu_item_background_color_focus);
    KLBUI_GLOBAL_menu_bind("item.background-color:disabled", globalcss_klbui_menu_item_background_color_disable);
}


//////////////////////////////////////////////////////////////////////////
// 仿 CSS 方法 -- 私有CSS属性

static void on_klbui_menu_margin(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_margin(&(p_css->margin), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_margin_top(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_margin_top(&(p_css->margin), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_margin_right(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_margin_right(&(p_css->margin), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_margin_bottom(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_margin_bottom(&(p_css->margin), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_margin_left(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_margin_left(&(p_css->margin), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_padding(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_padding(&(p_css->padding), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_padding_top(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_padding_top(&(p_css->padding), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_padding_right(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_padding_right(&(p_css->padding), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_padding_bottom(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_padding_bottom(&(p_css->padding), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_padding_left(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_padding_left(&(p_css->padding), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_text_color(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_text_color(&(p_css->normal.text), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_text_align(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_text_align(&(p_css->normal.text), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_font_size(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_font_size(&(p_css->normal.font), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_background_color(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_background_color(&(p_css->normal.background), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_background_image(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_background_image(&(p_css->normal.background), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_border_width(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_border_width(&(p_css->normal.border), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_border_color(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_border_color(&(p_css->normal.border), p_wnd, method, p_in, p_out);
}

////////////////////////////////////////
// 子项

static void on_klbui_menu_item_text_color(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_text_color(&(p_css->css_item.normal.text), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_item_text_color_focus(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_text_color(&(p_css->css_item.focus.text), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_item_text_color_disable(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_text_color(&(p_css->css_item.disable.text), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_item_text_align(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_text_align(&(p_css->css_item.normal.text), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_item_text_align_focus(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_text_align(&(p_css->css_item.focus.text), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_item_text_align_disable(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_text_align(&(p_css->css_item.disable.text), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_item_font_size(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_font_size(&(p_css->css_item.normal.font), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_item_font_size_focus(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_font_size(&(p_css->css_item.focus.font), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_item_font_size_disable(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_font_size(&(p_css->css_item.disable.font), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_item_background_color(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_background_color(&(p_css->css_item.normal.background), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_item_background_color_focus(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_background_color(&(p_css->css_item.focus.background), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_item_background_color_disable(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbwnd_menu_css_t* p_css = check_css_klbui_menu(p_wnd, method);
    klbuicssex_background_color(&(p_css->css_item.disable.background), p_wnd, method, p_in, p_out);
}

//////////////////////////////////////
// 自定义属性

static void on_klbui_menu_value(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    klbuicssex_attribute_sds(&(p_menu->menu.value), p_wnd, method, p_in, p_out);
}

static void on_klbui_menu_append(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    if (KLBUI_CSSEX_set == method)
    {
        /* eg.
            jq('menu1').append({
                {['value']=['1'], ['title']='项目1'},
                {['value']=['2'], ['title']='项目2'},
                {['value']=['3'], ['title']='项目3'},
                {['value']=['4'], ['title']='项目4'},
            })

            or, kgui.set('/xxx/.../combo1', 'append', {
                {['value']=['1'], ['title']='项目1'},
                {['value']=['2'], ['title']='项目2'},
                {['value']=['3'], ['title']='项目3'},
                {['value']=['4'], ['title']='项目4'},
            })
        */
        int start = 1;
        klb_map_t* p_in_array = (klb_map_t*)klb_map_idx_to_map(p_in, start);
        if (NULL != p_in_array)
        {
            klbwnd_menu_append(p_wnd, p_in_array);

            klb_wnd_update(p_wnd);
        }
    }
}

static void on_klbui_menu_append_2rd(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    if (KLBUI_CSSEX_set == method)
    {
        /* eg.
            jq('menu1').append_2rd({
                ['1'] = {
                    {['value']=['1-1'], ['title']='子项目1-1'},
                    {['value']=['1-2'], ['title']='子项目1-2'},
                    {['value']=['1-3'], ['title']='子项目1-3'},
                    {['value']=['1-4'], ['title']='子项目1-4'},
                },
                ['2'] = {
                    {['value']=['2-1'], ['title']='子项目2-1'},
                    {['value']=['2-2'], ['title']='子项目2-2'},
                    {['value']=['2-3'], ['title']='子项目2-3'},
                    {['value']=['2-4'], ['title']='子项目2-4'},
                },
            })
        */
        int start = 1;
        klb_map_t* p_in_2rd = (klb_map_t*)klb_map_idx_to_map(p_in, start);
        if (NULL != p_in_2rd)
        {
            klbwnd_menu_append_2rd(p_wnd, p_in_2rd);
        }
    }
}

static void on_klbui_menu_item_update(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    // 更新 第一级菜单项目
    if (KLBUI_CSSEX_set == method)
    {
        /* eg.
        jq('menu1')['item.update']({
            {['value']=['1'], ['title']='项目1'},
            {['value']=['2'], ['title']='项目2'},
            {['value']=['3'], ['title']='项目3'},
            {['value']=['4'], ['title']='项目4'},
        })

        or, kgui.set('/xxx/.../combo1', 'append', {
            {['value']=['1'], ['title']='项目1'},
            {['value']=['2'], ['title']='项目2'},
            {['value']=['3'], ['title']='项目3'},
            {['value']=['4'], ['title']='项目4'},
        })
        */
        int start = 1;
        klb_map_t* p_in_array = (klb_map_t*)klb_map_idx_to_map(p_in, start);
        if (NULL != p_in_array)
        {
            klbwnd_menu_item_update(p_wnd, p_in_array);

            klb_wnd_update(p_wnd);
        }
    }
}

static void on_klbui_menu_item_2rd_update(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, int method, const klb_map_t* p_in, klb_map_t* p_out)
{
    // 更新 第二级菜单项目
    if (KLBUI_CSSEX_set == method)
    {
        /* eg.
        jq('menu1')['item_2rd.update']({
            ['1'] = {
                {['value']=['1-1'], ['title']='子项目1-1'},
                {['value']=['1-2'], ['title']='子项目1-2'},
                {['value']=['1-3'], ['title']='子项目1-3'},
                {['value']=['1-4'], ['title']='子项目1-4'},
            },
            ['2'] = {
                {['value']=['2-1'], ['title']='子项目2-1'},
                {['value']=['2-2'], ['title']='子项目2-2'},
                {['value']=['2-3'], ['title']='子项目2-3'},
                {['value']=['2-4'], ['title']='子项目2-4'},
            },
        })
        */
        int start = 1;
        klb_map_t* p_in_2rd = (klb_map_t*)klb_map_idx_to_map(p_in, start);
        if (NULL != p_in_2rd)
        {
            klbwnd_menu_item_2rd_update(p_wnd, p_in_2rd);
        }
    }
}

//////////////////////////////////////////////////////////////////////////
// css func

#define KLBUI_menu_bind(KEY_, FUNC_) { klb_map_set_ptr(p_menu->p_func_map, (KEY_), (void*)(FUNC_), NULL); }

static void klbui_menu_init_func_map(klb_wnd_t* p_wnd, klbui_menu_t* p_menu, klb_gui_t* p_gui)
{
    klb_map_t* ptr = klb_gui_css_map(p_gui, KLBWUI_kmenu);
    if (NULL != ptr)
    {
        p_menu->p_func_map = ptr;
        return;
    }

    // 未找到, 则新添加 解析map, 及处理函数
    ptr = klb_gui_new_css_map(p_gui, KLBWUI_kmenu);
    p_menu->p_func_map = ptr;


    //////////////////////////////////////////////
    // 若第一次, 则添加全局属性解析方法
    // 仿 CSS 方法

    // 外边距 margin
    KLBUI_menu_bind("margin", on_klbui_menu_margin);
    KLBUI_menu_bind("margin-top", on_klbui_menu_margin_top);
    KLBUI_menu_bind("margin-right", on_klbui_menu_margin_right);
    KLBUI_menu_bind("margin-bottom", on_klbui_menu_margin_bottom);
    KLBUI_menu_bind("margin-left", on_klbui_menu_margin_left);

    // 内边距 padding
    KLBUI_menu_bind("padding", on_klbui_menu_padding);
    KLBUI_menu_bind("padding-top", on_klbui_menu_padding_top);
    KLBUI_menu_bind("padding-right", on_klbui_menu_padding_right);
    KLBUI_menu_bind("padding-bottom", on_klbui_menu_padding_bottom);
    KLBUI_menu_bind("padding-left", on_klbui_menu_padding_left);

    // 文本颜色 color
    KLBUI_menu_bind("color", on_klbui_menu_text_color);

    // 文本对齐 text-align
    KLBUI_menu_bind("text-align", on_klbui_menu_text_align);

    // 字体大小 font-size
    KLBUI_menu_bind("font-size", on_klbui_menu_font_size);

    // 背景色 background-color
    KLBUI_menu_bind("background-color", on_klbui_menu_background_color);

    // 背景图片 background-image
    KLBUI_menu_bind("background-image", on_klbui_menu_background_image);

    // 边框的宽度 border-width
    KLBUI_menu_bind("border-width", on_klbui_menu_border_width);

    // 边框的颜色 border-color
    KLBUI_menu_bind("border-color", on_klbui_menu_border_color);

    //////////////////////////////////////////////
    // 子项

    // 子项.文本颜色 color
    KLBUI_menu_bind("item.color", on_klbui_menu_item_text_color);
    KLBUI_menu_bind("item.color:focus", on_klbui_menu_item_text_color_focus);
    KLBUI_menu_bind("item.color:disabled", on_klbui_menu_item_text_color_disable);

    // 子项.文本对齐 text-align
    KLBUI_menu_bind("item.text-align", on_klbui_menu_item_text_align);
    KLBUI_menu_bind("item.text-align:focus", on_klbui_menu_item_text_align_focus);
    KLBUI_menu_bind("item.text-align:disabled", on_klbui_menu_item_text_align_disable);

    // 子项.字体大小 font-size
    KLBUI_menu_bind("item.font-size", on_klbui_menu_item_font_size);
    KLBUI_menu_bind("item.font-size:focus", on_klbui_menu_item_font_size_focus);
    KLBUI_menu_bind("item.font-size:disabled", on_klbui_menu_item_font_size_disable);

    // 子项.背景色 background-color
    KLBUI_menu_bind("item.background-color", on_klbui_menu_item_background_color);
    KLBUI_menu_bind("item.background-color:focus", on_klbui_menu_item_background_color_focus);
    KLBUI_menu_bind("item.background-color:disabled", on_klbui_menu_item_background_color_disable);

    //////////////////////////////////////////////
    // 自定义方法

    KLBUI_menu_bind("value", on_klbui_menu_value);

    KLBUI_menu_bind("append", on_klbui_menu_append);
    KLBUI_menu_bind("append_2rd", on_klbui_menu_append_2rd);

    KLBUI_menu_bind("update", on_klbui_menu_item_update);
    KLBUI_menu_bind("update_2rd", on_klbui_menu_item_2rd_update);
}

//////////////////////////////////////////////////////////////////////////
// create, register

static klb_wnd_t* klbui_menu_create(klb_gui_t* p_gui, int x, int y, int w, int h)
{
    // step1. malloc
    klb_wnd_t* p_wnd = KLB_MALLOCZ(klb_wnd_t, 1, sizeof(klbui_menu_t));
    klbui_menu_t* p_menu = (klbui_menu_t*)p_wnd->ctrl;

    // step2. 初始化基础部分
    klbwnd_menu_init(p_wnd, p_gui, x, y, w, h);

    // step3. 重写部分函数
    p_wnd->vtable.destroy = klbui_menu_destroy;
    p_wnd->vtable.on_set = klbui_menu_on_set;
    p_wnd->vtable.on_get = klbui_menu_on_get;

    // step4. 初始化CSS 支持的方法
    klbui_menu_init_func_map(p_wnd, p_menu, p_gui);

    // step5. 默认使用公用全局CSS属性
    p_menu->p_globalcss = (klbwnd_menu_css_t*)klb_gui_globalcss_get_ptr(p_gui, KLBWUI_kmenu);
    klbwnd_menu_set_css(p_wnd, p_menu->p_globalcss);

    return p_wnd;
}

// 注册 "kmenu"
int klbui_register_kmenu(klb_gui_t* p_gui)
{
    klbui_menu_init_globalcss(p_gui); // 初始化全局CSS, 需要注册时初始化, 否则全局属性无法生效
    return klb_gui_register(p_gui, KLBWUI_kmenu, klbui_menu_create);
}

// end
