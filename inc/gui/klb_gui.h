...
KLB_API int klb_gui_start(klb_gui_t* p_gui);


/// @brief 关闭线程
/// @param [in] *p_gui          GUI对象
/// @return 无
KLB_API void klb_gui_stop(klb_gui_t* p_gui);
...
///  \n 与 start/stop 不能同时使用
///  \n 当有消息(事件)时调用; 定时器定时调用
KLB_API int klb_gui_process_message(klb_gui_t* p_gui, uint64_t tick_count);
