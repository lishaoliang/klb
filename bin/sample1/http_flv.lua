...
local khttp_flv = require("khttp_flv")
...
local send_media_http_flv = function (conn)
	-- head
...
local dispatch_http_flv = function (conn)
	local url = ''
	local method = ''
	...
	while true do
		...
		elseif 'body' == t then
			--if 'GET' == method then
				print('http-flv', 'GET', url)
				send_media_http_flv(conn)
			--end
		elseif 'disconnect' == t then
			break
		end
...
local listen_http_flv = khttp_flv.listen(port + 1)

kco.fork(function ()
	while true do
		local conn = listen_http_flv:co_accept()
		kco.fork(function ()
			print('http-flv connect', conn)
			
			dispatch_http_flv(conn)
			
			print('http-flv disconnect', conn)
			conn:disconnect()
		end)
	end
end)
...
kco.timeout(1000, function ()
	local conn = khttp_flv.connect('127.0.0.1', port + 1)

	

	conn:send_text(pack_req_get('/httpflv', '127.0.0.1' .. tostring(port + 1)), '')
