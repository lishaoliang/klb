...
func (m *Conn) doReadWebsocket() error {
	...
	if wsStatusHeader == m.wsStatus {
		...
		spare := buf.Spare()
		if len(spare) <= 0 {
			return errors.New("kwsconn.conn_rws.doReadWebsocket,out of memory,read header")
		}
	...
	spare := buf.Spare()
	if len(spare) <= 0 {
		return errors.New("kwsconn.conn_rws.doReadWebsocket,out of memory,read body")
	}
...
func (m *Conn) onReadFrame(ws kws.Ws, b []byte) {
	...
	case kws.WsOpcodeClose:
		m.conn.PushDisconnect(errors.New("kws.kwsconn.conn_rws.onReadFrame,recv opcode close"))
	case kws.WsOpcodePing:
		fmt.Println("kwsconn.onReadFrame Ping")
		//m.conn.SendHeart()
	case kws.WsOpcodePong:
		fmt.Println("kwsconn.onReadFrame Pong")
	}
}
