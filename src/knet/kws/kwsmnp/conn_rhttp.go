...
func (m *Conn) doReadHTTP() error {
	...
	if httpStatusHeader == m.httpStatus {
		...
		spare := buf.Spare()
		if len(spare) <= 0 {
			return errors.New("kwsmnpconn.conn_rhttp.doReadHTTP,out of memory,read header")
		}
	...
	spare := buf.Spare()
	if len(spare) <= 0 {
		return errors.New("kwsmnpconn.conn_rhttp.doReadHTTP,out of memory,read body")
	}
