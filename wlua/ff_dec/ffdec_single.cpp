...
ff_dec_single_t* ff_dec_single_create()
{
    ...
    p_ff_dec->p_video_list = klb_nlist_create();
    p_ff_dec->p_yuv_list = klb_nlist_create();
    ...
    {
        p_ff_dec->p_rate_ctrl[i] = ff_rate_ctrl_create(p_ff_dec, (ff_rate_ctrl_type_e)i);
    }
    p_ff_dec->p_rate_ctrl_active = p_ff_dec->p_rate_ctrl[FF_RATE_CTRL_LOW];


    // 音频
    p_ff_dec->p_audio_list = klb_nlist_create();
    p_ff_dec->p_wav_list = klb_nlist_create();
...
void ff_dec_single_destroy(ff_dec_single_t* p_ff_dec)
{
...
#if 0
    while (0 < klb_nlist_size(p_ff_dec->p_yuv_list))
    {
        em_frame_yuv_wav_t* p_yuv = (em_frame_yuv_wav_t*)klb_nlist_pop_head(p_ff_dec->p_yuv_list);
        KLB_FREE_BY(p_yuv, em_frame_yuv_wav_free);
    }

    while (0 < klb_nlist_size(p_ff_dec->p_video_list))
    {
        em_buf_t* p_buf = (em_buf_t*)klb_nlist_pop_head(p_ff_dec->p_video_list);
        KLB_FREE_BY(p_buf, em_buf_unref_next);
    }

    while (0 < klb_nlist_size(p_ff_dec->p_wav_list))
    {
        em_frame_yuv_wav_t* p_wav = (em_frame_yuv_wav_t*)klb_nlist_pop_head(p_ff_dec->p_yuv_list);
        KLB_FREE_BY(p_wav, em_frame_yuv_wav_free);
    }

    while (0 < klb_nlist_size(p_ff_dec->p_audio_list))
    {
        em_buf_t* p_buf = (em_buf_t*)klb_nlist_pop_head(p_ff_dec->p_video_list);
        KLB_FREE_BY(p_buf, em_buf_unref_next);
    }
    ...
    KLB_FREE_BY(p_ff_dec->p_wav_list, klb_nlist_destroy);
    KLB_FREE_BY(p_ff_dec->p_audio_list, klb_nlist_destroy);
    KLB_FREE_BY(p_ff_dec->p_yuv_list, klb_nlist_destroy);
    KLB_FREE_BY(p_ff_dec->p_video_list, klb_nlist_destroy);
...
int ff_dec_single_push(ff_dec_single_t* p_ff_dec, klb_buf_t* p_buf)
{
    assert(NULL != p_ff_dec);
    ...
    {
        klb_nlist_push_tail(p_ff_dec->p_video_list, p_buf);
    }
    else if (KLB_MNP_DTYPE_AAC == p_media->dtype)
    {
        klb_nlist_push_tail(p_ff_dec->p_audio_list, p_buf);
    }
    else
    {
        //KLB_FREE_BY(p_buf, em_buf_unref_next);
...
{
    em_buf_t* p_video = (em_buf_t*)klb_nlist_head(p_ff_dec->p_video_list);
    em_buf_t* p_audio = (em_buf_t*)klb_nlist_head(p_ff_dec->p_audio_list);
    ...
    if (NULL == p_video || NULL == p_audio)
    {
        ...
        if (NULL != p_video)
        {
            return (em_buf_t*)klb_nlist_pop_head(p_ff_dec->p_video_list);
        }

        if (NULL != p_audio)
        {
            return (em_buf_t*)klb_nlist_pop_head(p_ff_dec->p_audio_list);
        }
...
static void ff_dec_manage_drop_data(ff_dec_single_t* p_ff_dec, bool hidden)
{
    ...
    if (0 < drop_num)
    {
        ...
        klb_list_iter_t* p_iter = klb_nlist_begin(p_ff_dec->p_yuv_list);
        while (NULL != p_iter)
        {
            em_frame_yuv_wav_t* p_yuv = (em_frame_yuv_wav_t*)klb_nlist_data(p_iter);
            p_yuv->pts = 0;
...
#if 0
    while (klb_nlist_size(p_ff_dec->p_yuv_list) < yuv_max)
    {
                ...
                KLB_MNP_DTYPE_H265 == p_media->dtype)
            {
                ...
                if (!p_ff_dec->need_key)
                {
                    ...
                    if (NULL != p_dec)
                    {
                        ...
                        if (NULL != p_yuv)
                        {
                            klb_nlist_push_tail(p_ff_dec->p_yuv_list, p_yuv);
                        }
                    }
                    else
                    {
