...
static em_frame_yuv_wav_t* ff_rate_ctrl_lower_get_frame(ff_rate_ctrl_t* p_rate_ctrl, int64_t now_ticks)
{
...
static int ff_rate_ctrl_lower_drop_data(ff_rate_ctrl_t* p_rate_ctrl, bool hidden)
{
    ...
    else
    {
        ...
        // 此时也需要丢帧
        if (FF_RATE_CTRL_LOWER_FRAME_MAX < klb_nlist_size(p_ff_dec->p_video_list))
        {
            drop_num = ff_rate_ctrl_drop_to_last_key(p_ff_dec);
        }
    }
