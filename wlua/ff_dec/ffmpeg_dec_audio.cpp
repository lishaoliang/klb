...
ffmpeg_dec_audio_t* ffmpeg_dec_audio_create(enum AVCodecID id /*=AV_CODEC_ID_AAC*/, int tracks, int samples, int bits_per_coded_sample)
{
    ...
    p_dec->p_codec = (AVCodec*)avcodec_find_decoder(codec_id);
...
static void ffmpeg_dec_audio_push(ffmpeg_dec_audio_t* p_dec, const char* p_data, int data_len, int64_t pts, klb_nlist_t* p_list)
{
    ...
    klb_nlist_push_tail(p_list, p_wav);
}

int ffmpeg_dec_audio_decode(ffmpeg_dec_audio_t* p_dec, uint8_t* p_data, int data_len, int64_t pts, klb_nlist_t* p_list)
{
    assert(NULL != p_dec);
    assert(NULL != p_data);
    assert(NULL != p_list);
    ...
    while (0 < data_len)
    {
        ...
        if (dec_len < 0)
        {
            KLB_LOG("C avcodec_decode_audio4 error!");
            assert(false);
